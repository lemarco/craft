...
#   1) seeded trembita-sim linearizability sweep (checker in-process)
...
#   TREMBITA_E2E_LINEARIZABILITY=1 ./e2e/linearizability.sh  # CI nightly

set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

if [ "${TREMBITA_LINEARIZABILITY_SKIP_SIM:-0}" != "1" ]; then
  echo "=== phase 1: trembita-sim linearizability (seed sweep) ==="
  ./scripts/test-with-log.sh -p trembita-sim --test linearizability 2>&1 | tail -5

  SEEDS="${TREMBITA_LINEARIZABILITY_SEEDS:-42 99 1234}"
  for seed in $SEEDS; do
    echo "  read_index adversarial seed=$seed"
    TREMBITA_SIM_SEED="$seed" ./scripts/test-with-log.sh -p trembita-sim --test read_index 2>&1 | tail -3
  done
else
  echo "SKIP phase 1 (TREMBITA_LINEARIZABILITY_SKIP_SIM=1)"
fi

if [ "${TREMBITA_E2E_LINEARIZABILITY:-1}" != "1" ]; then
  echo "SKIP phase 2 (set TREMBITA_E2E_LINEARIZABILITY=1 to enable docker E2E gate)"
...
echo "--- 2a: concurrent QUIC inc/read + trembita_sim checker (healthy cluster) ---"
