...
# NodeId -> host-mapped ops HTTP (TCP on TREMBITA_LISTEN inside compose; see docker-compose.yml).
...
# Run concurrent QUIC inc/read rounds and check with trembita_sim::History (phase 2).
run_linclient() {
    $COMPOSE --profile linclient run --rm linclient
}

# Run the QUIC job-queue client (`TREMBITA_E2E_QUEUE_PHASE` = before/after_failover).
run_queue_client() {
    local phase="$1"
    TREMBITA_E2E_QUEUE_PHASE="$phase" $COMPOSE --profile queueclient run --rm queueclient
}
