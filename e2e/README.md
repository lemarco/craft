...
A 3-node `trembita` cluster over **real QUIC + mTLS**, exercised end to end in
containers. This is the highest-fidelity test in the suite: it
uses the actual `trembita-node` binary, the actual cert-provisioning script, and a
real network between separate processes.

## Layout

- `Dockerfile` — builds the `trembita-node` binary and a small runtime image
...
- `cert_renew.sh` — PEM hot reload (cert-automation): reissues a follower's on-disk
  ...
  (`TREMBITA_CERT_WATCH_SECS`), and asserts the cluster stays healthy without
  process restart.
...
(`TREMBITA_E2E_PUMBA=1`) that adds delay + jitter to every node and asserts
