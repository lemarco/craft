...
GATEWAYS="${CRAFTY_GATEWAYS:-http://127.0.0.1:8090 http://127.0.0.1:8091 http://127.0.0.1:8092}"
...
for i in $(seq 1 "$COUNT"); do
  gw="${GW[$(( (i - 1) % ${#GW[@]} ))]}"
  code=$(curl -s -o /tmp/crafty-job-$$.json -w '%{http_code}' -X POST "$gw/jobs/emails" \
