...
run_autofix_commit() {
  ...
  if [[ "$TIER" == "push" || "$TIER" == "release" ]]; then
    if [[ "${TREMBITA_NO_AUTOFIX_COMMIT:-0}" == "1" || "$NO_AUTOFIX_COMMIT" == "1" ]]; then
      return 0
    fi
...
run_push_steps() {
  if [[ "${TREMBITA_GATE_AUTOFIX:-0}" == "1" ]]; then
    run_autofix_commit
  fi
  ...
  if [[ "$RELEASE_BUILD" == "1" || "${TREMBITA_SKIP_RELEASE:-1}" != "1" ]]; then
    bash scripts/gate-step.sh release-build
  else
    log "release build skipped (use --release-build or TREMBITA_SKIP_RELEASE=0)"
  fi
}
...
case "$TIER" in
  ...
  push)
    ...
    ;;
  release)
    export TREMBITA_MSRV_STRICT=1
    export TREMBITA_GATE_AUTOFIX=1
    STAGED_ONLY=0
