# ADR 018: ClusterSupervisor — leader-only reconciliation
...
Open question **#6**: who runs cluster-wide actor placement — auto-spawn on join ([ADR 015](015-auto-spawn-on-join.md)), `scale_cluster`, migration ([ADR 013](013-cross-node-actors.md))?
...
- **Membership change committed** ([ADR 016](016-membership-early.md)) → reconcile auto workers for new/removed nodes
- **`scale_cluster` API call** (any node forwards to leader) → leader computes plan → issues spawns/stops
- **`leave()` / node failure detected** → leader plans migration → `/actor/migrate` ([ADR 013](013-cross-node-actors.md))
...
- Non-leaders receiving `scale_cluster` or post-join callbacks **forward to leader** (same pattern as client forward, [ADR 003](003-client-routing.md)).
- During election: operations return `503` / retry until leader elected.

### Idempotency

Leader reconciliation is **declarative**: desired state = N auto workers on N nodes ([ADR 014](014-one-worker-per-vps.md)); diff vs directory; idempotent spawns by `(name, node_id, generation)`.
...
- Safe auto-spawn on join with [ADR 015](015-auto-spawn-on-join.md)
