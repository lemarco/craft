# ADR 033: Per-group Raft membership (multi-Raft)
...
[ADR 031](031-write-sharding-multi-raft.md) landed multi-Raft runtime wiring: N
...
[ADR 027](027-future-work-and-risks.md)).
...
The target model in ADR 031 § "Membership & placement" is **joint consensus per
group** ([ADR 016](016-membership-early.md)): each Raft group owns its voter
...
(ADR 031) and group membership changes are **orthogonal but coordinated**:
...
([ADR 031](031-write-sharding-multi-raft.md)) to move a group's *state* before
...
| Cross-node group migration RPC before retire | **Landed (ADR 031)** |
...
- Planner is pure, deterministic, and testable — same pattern as ADR 031 routing.
...
- **Phi-accrual or SWIM for group membership.** Rejected — membership stays
  log-authoritative ([ADR 016](016-membership-early.md), [ADR 032](032-liveness-vs-membership.md)).

## Related

- [ADR 031](031-write-sharding-multi-raft.md) — multi-Raft runtime, deferred items
- [ADR 016](016-membership-early.md) — joint consensus
- [ADR 018](018-supervisor-leader.md) — leader-owned control plane
- [ADR 032](032-liveness-vs-membership.md) — liveness vs committed voters
