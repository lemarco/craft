...
With the [Rust-native client API](client-api.md), a client may connect to **any** node via HTTP/3. Only the leader can append to the log (and serve linearizable queries per [read-consistency](read-consistency.md)). When a **follower** receives `ClientRequest`, we must define behavior.
...
- Resolved to `SocketAddr` via static cluster config ([discovery](discovery.md))
