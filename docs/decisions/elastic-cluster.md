# Elastic cluster — incremental VPS join & actor scaling
...
[deployment-model](deployment-model.md): users deploy the **same app** to multiple VPSes. **Actors scale on demand** across the cluster ([cross-node-actors](cross-node-actors.md)). **Production: one worker per VPS** ([one-worker-per-vps](one-worker-per-vps.md)). **Auto-spawn workers on join** ([auto-spawn-on-join](auto-spawn-on-join.md)).
...
User **does not** need `spawn` in `main` for declared auto workers ([auto-spawn-on-join](auto-spawn-on-join.md)).
