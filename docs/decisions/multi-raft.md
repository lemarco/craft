...
| Modulus shard expansion | `ShardRouter::expand_shard_count`, `CraftCluster::expand_shard_count` — keys remap; prefer add groups |
...
- Facade: `CraftCluster::add_raft_groups(count)`.
...
**Default path:** framework saga coordinator with `SagaStep { key, command, compensate }`, `SagaJournal`, metrics (`craft_saga_*`). Compensation runs on the **same shard as the forward step**.
...
|---------|----------------|
| Reachability tuning + hysteresis | `ReachabilityConfig`, `CraftClusterBuilder::reachability()` |
| Phi-accrual detector | `FailureDetectorKind::PhiAccrual` |
| Snapshot backup / restore | `craft-ops` CLI — local gzip-tar + `s3://` / `gs://` / `file://` via opendal |
| Rolling wire upgrade (N/N−1) | `MIN_COMPATIBLE_PROTOCOL_VERSION` + `protocol_version_compatible()` |
| Admin TLS | `AdminServer::serve_tls`, `CRAFT_ADMIN_TLS_*` |
| Jepsen-lite gate | `e2e/linearizability.sh` — craft-sim checker + docker phase |
