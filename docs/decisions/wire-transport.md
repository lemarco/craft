# Wire transport — HTTP/3 everywhere
...
[client-api](client-api.md) rejected gRPC in favor of Rust-native types. We considered framed TCP + `postcard` vs HTTP/3 for the client edge only. The user chose **HTTP/3 for all wire traffic** — Raft peer RPC, snapshots, and remote client API share one transport stack.
...
POST /raft/v1/client/wire   # client API; TLS (mTLS optional — see security)
```
...
// Client path uses ClientRequest in / ClientResponse out (client-api)
```
...
| HTTP | Meaning |
|------|---------|
| `200` + `application/x-postcard` | `ClientResponse` — handled or proxied ([client-routing](client-routing.md)) |
...
- Built-in encryption and multiplexing (addresses much of security)
...
- **Peer RPC uses a dedicated QUIC connection** separate from client/actor traffic ([future-work-and-risks](future-work-and-risks.md) R2) — prevents heartbeat starvation under load
...
- [client-api.md](client-api.md) — client types and `RemoteClient`
- [security.md](security.md) — mTLS policy (updated)
