...
Medium open question **#5**: how users provision mTLS material for VPS deploys ([security](security.md)).
...
- Redis TLS (optional) — pointer only; user-managed ([actor-state-redis](actor-state-redis.md))

### Out of scope v1 (see [cert-automation](cert-automation.md) for automation)

- ACME, step-ca, cert-manager integration (defer → **done in cert-automation**)
