...
Add a [`ScheduleSource`](../../crates/trembita-actor/src/schedule_source.rs) port:
...
- **Leader only** — polled on the same loop as cron ticks ([`run_queue_schedule_ticker`](../../crates/trembita-actor/src/queue_schedule.rs))
...
- **`.cron()`** — unchanged API; implemented as [`StaticScheduleSource`](../../crates/trembita-actor/src/schedule_source.rs) merged into the same reconcile path
