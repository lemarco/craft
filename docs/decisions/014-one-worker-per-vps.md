# ADR 014: One worker per VPS (production)
...
[ADR 013](013-cross-node-actors.md) allows `spawn_pool`, `scale_local`, and `scale_cluster` to place multiple worker instances on one VPS. The user wants a different **production** model:
...
### Migration ([ADR 013](013-cross-node-actors.md))
