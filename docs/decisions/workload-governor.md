...
- Cross-node governor coordination (stay local fairness per VPS).
- OS cgroup / PSI integration in core — apps implement [`ExternalLoad`](external-load.md) instead.
...
- [external-load](external-load.md) — weighted tokens + `ExternalLoad` for subprocess CPU (B-17)
