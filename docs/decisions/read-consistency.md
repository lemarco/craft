...
Writes go through Raft log replication. **Reads** of authoritative state must define a consistency level. With [client-routing](client-routing.md), client queries are **forwarded to the leader**; this ADR defines what the leader does before answering.
...
Writes (`Propose`) still forward to the leader (client-routing). Lease reads remain
leader-only.
...
`StateMachine::query` remains **required** in v1 ([state-machine](state-machine.md)).
...
## Linearizability test plan ([testing-strategy](testing-strategy.md))
...
| **C — Defer reads** | Conflicts with `StateMachine::query` in state-machine |

## Related

- [client-routing.md](client-routing.md)
