# ADR 021: Stateful actors — external store (Redis)
...
| Layer | Store | Purpose |
|-------|--------|---------|
| **Authoritative / consensus** | Raft → `StateMachine` ([ADR 001](001-state-machine.md)) | Orders, balances, config — must be linearizable and replicated |
...
### Crash and migration ([ADR 013](013-cross-node-actors.md))

| Event | Behavior |
|-------|----------|
| **Graceful leave** | Drain in-flight; state already in Redis — new worker continues |
| **Crash** | Actor state in Redis **retained**; leader respawns worker on another VPS ([ADR 018](018-supervisor-leader.md)); worker **reloads from Redis** |
