# ADR 030: Architecture style — pragmatic ports & adapters
...
- Persistence, transport, and external state are **traits** ([ADR 010](010-wire-transport.md), Track B, [ADR 021](021-actor-state-redis.md)).
- Deterministic simulation ([ADR 029](029-testing-strategy.md)) depends on swapping real adapters for in-memory/virtual ones.
...
A `Clock` port is injected wherever timing matters (election/heartbeat timers). The domain never calls `Instant::now()` or `tokio::time` directly — required for deterministic, seed-reproducible simulation ([ADR 029](029-testing-strategy.md)).
...
- [029-testing-strategy.md](029-testing-strategy.md) — why the pure core matters
- [010-wire-transport.md](010-wire-transport.md) · [021-actor-state-redis.md](021-actor-state-redis.md)
