# ADR 002: Client API — Rust-native, no gRPC

**Status:** Accepted  
**Date:** 2026-07-05  
**Amended:** 2026-07-05 — remote wire uses HTTP/3 ([ADR 010](010-wire-transport.md))
...
| **L2 — Remote** | **HTTP/3** + `postcard` body ([ADR 010](010-wire-transport.md)) | External Rust clients, CLI, other binaries |

Both layers share **`raft-proto` client message types** (`ClientRequest`, `ClientResponse`, …). Only the transport differs.

## Why this stack

**Unified with Raft wire ([ADR 010](010-wire-transport.md))**
...
/// Remote: HTTP/3 client to any cluster member; followers forward to leader (ADR 003)
...
Followers **forward** to leader; clients do not handle NotLeader for normal operation (see ADR 003).
```
...
|--------|---------|
| **gRPC (tonic)** | Rejected |
| **Framed TCP + postcard** | Rejected — user chose HTTP/3 everywhere ([010](010-wire-transport.md)) |
...
- [010-wire-transport.md](010-wire-transport.md) — HTTP/3 for peer RPC too
- [003-client-routing.md](003-client-routing.md) — transparent forward from followers
- [006-security.md](006-security.md) — mTLS for peer path
