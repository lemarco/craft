# Auto-spawn workers on VPS join
...
[one-worker-per-vps](one-worker-per-vps.md): production = **1 worker per VPS**. [elastic-cluster](elastic-cluster.md): users deploy new VPSes with `JOIN_ADDR`. When a node joins the cluster, something must spawn the worker on that node.
...
| **Node leaves** | Migrate/stop per [cross-node-actors](cross-node-actors.md); supervisor may respawn on remaining nodes via `scale_cluster` |
| **New node added, count should rise** | Auto workers on new node; `scale_cluster` target may track `live_node_count` |

### Production rules ([one-worker-per-vps](one-worker-per-vps.md))
