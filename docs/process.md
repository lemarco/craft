...
| **heavy** (e2e, **local-elastic-smoke**, store-redis, capstore-postgres) | nightly schedule **or** MR label `run-heavy` | [`ci-local-elastic-smoke.sh`](../scripts/ci-local-elastic-smoke.sh) (B-50 laptop elastic, no compose stack) · e2e scripts / optional store integration tests |
