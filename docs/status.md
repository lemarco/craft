...
- **`JobQueue`** — `InMemoryJobQueue`, `RedbJobQueue`, leader `QueueService`, `ClusterJobQueue`, autoscale ([job-queue](decisions/job-queue.md))
...
- **Real-world soak** — harness exists (`benchmarks/soak`); long-running production soak is operator responsibility
