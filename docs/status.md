...
| **crates.io** | [`0.6.3`](https://crates.io/crates/trembita) · [release notes](releases/0.6.3.md) |
| **MSRV** | 1.94 |
...
**Not goals:** linearizable actor `ask`, global cross-shard serializable isolation. **Open epics:** none in [backlog § Open work](backlog.md#open-work) (2026-09-17) — deferred ADR topics only.
...
Shipped backlog **B-28 … B-54** (table below). Core theme: **«add VPS + same binary»** — capability compute, gateway sessions, ingress health checks, scale-safe manifests, coordination-scale queues / multi-Raft, then ops/deploy/observability through **B-54**. **Planning archives:** [B-28–B-32](archive/backlog-wave-b28-b32.md) · [B-33–B-41](archive/backlog-wave-b33-b41.md). **B-53** closed doc/index hygiene for the B-33–B-41 wave ([archive](archive/backlog-wave-b33-b41.md) + [testing-coverage § B-33–B-41](testing-coverage.md#shipped-backlog-b-33b41)).
...
| **B-42** | **Local elastic parity** — staged **4th joiner**, nginx **3/4** upstreams, `elastic-smoke` / **`GET /e2e/whoami`** on **realtime** (mirrors [B-34](scenarios/capabilities.md#elastic-join--lb-b-34) on a laptop) | [local-3node § B-42](../dev/local-3node/README.md#local-elastic-parity-b-42) · [capabilities § B-42](scenarios/capabilities.md#local-elastic-parity-b-42) · [capability-dx § B-42](decisions/capability-dx.md#local-elastic-parity-b-42) | [`cluster.rs`](../crates/trembita-cli/src/dev/cluster.rs), [`local_cluster.rs`](../crates/trembita-cli/src/dev/local_cluster.rs), [`showcases.rs`](../crates/trembita-cli/src/dev/showcases.rs), [`tests/dev.rs`](../crates/trembita-cli/tests/dev.rs), [`e2e_elastic/`](../crates/trembita-tools/src/e2e_elastic/), [`examples/realtime`](../examples/realtime/src/main.rs), [`local-cluster.sh`](../scripts/local-cluster.sh) — **`b42_*`** index: [capabilities § regression](scenarios/capabilities.md#automated-regression-b-42) |
| **B-52** | **Domain DX patterns** — outbox / idempotency / consensus helper docs + scaffold samples (`task.rs`, `domain/outbox.rs`, `consensus.rs`) | [capability-dx § B-52](decisions/capability-dx.md#domain-dx-patterns-b-52) · [structural-limits § B-52](scenarios/structural-limits.md#domain-patterns-b-52) · [capabilities § B-52](scenarios/capabilities.md#domain-dx-patterns-b-52) | [`scaffold/render.rs`](../crates/trembita-cli/src/scaffold/render.rs), [`tests/scaffold.rs`](../crates/trembita-cli/tests/scaffold.rs) (`b52_*`) |
| **B-53** | **Backlog / coverage hygiene** — `archive/backlog-wave-b33-b41.md`; [testing-coverage](testing-coverage.md) shipped index split (B-28–B-32 · B-33–B-41 · B-42+) | [backlog](backlog.md) header · [archive B-33–B-41](archive/backlog-wave-b33-b41.md) · [capabilities § B-53](scenarios/capabilities.md#backlog-coverage-hygiene-b-53) | [`deploy_pack.rs`](../crates/trembita/tests/deploy_pack.rs) (`b53_*`) |
| **B-51** | **Ops observability layer** — `OtlpMetricsSink`, auto-wire on `from_env` + `otlp-metrics`, `trembita_join_*` gauges, documented alert thresholds | [runbook § B-51](ops/production-runbook.md#ops-observability-layer-b-51) · [env § OTEL](env.md#opentelemetry-metrics-b-51) · [observability](decisions/observability.md) | [`trembita-metrics-otlp`](../crates/trembita-metrics-otlp/) (`b51_*`), [`join_pipeline.rs`](../crates/trembita-assembly/src/join_pipeline.rs) (`b51_*`) |
| **B-50** | **CI local elastic smoke** — GitLab **`local-elastic-smoke`** heavy job → [`scripts/ci-local-elastic-smoke.sh`](../scripts/ci-local-elastic-smoke.sh) (`local-cluster.sh` elastic-up + lb-up + elastic-smoke); faster than full [`e2e/elastic_lb.sh`](../e2e/elastic_lb.sh) | [capabilities § B-50](scenarios/capabilities.md#ci-local-elastic-smoke-b-50) · [process § heavy](process.md#ci-lanes) · [local-3node § B-42](../dev/local-3node/README.md#local-elastic-parity-b-42) | [`ci-local-elastic-smoke.sh`](../scripts/ci-local-elastic-smoke.sh), [`tests/dev.rs`](../crates/trembita-cli/tests/dev.rs) (`b50_*`), [`.gitlab-ci.yml`](../.gitlab-ci.yml) |
| **B-54** | **CI cluster upgrade operator** — `trembita-ops upgrade run`: preflight `/ready` + `/cluster/upgrade`, optional ops-summary warn, POST desired, poll `fleet_ready`, post smoke | [capabilities § B-54](scenarios/capabilities.md#ci-cluster-upgrade-operator-b-54) · [runbook § B-54](ops/production-runbook.md#ci-cluster-upgrade-operator-b-54) · [deploy/rolling-upgrade-systemd § C](../deploy/rolling-upgrade-systemd.md#c-external-operator-b-54) | [`ops/upgrade.rs`](../crates/trembita-tools/src/ops/upgrade.rs) (`b54_*`), [`upgrade_operator.rs`](../crates/trembita-tools/tests/upgrade_operator.rs) |
| **B-49** | **Rolling upgrade proof** — `rolling_upgrade_proof` tests, lab script, [deploy/rolling-upgrade-systemd § B-49](../deploy/rolling-upgrade-systemd.md#b-49--automated-proof-vs-manual-lab) | [capabilities § B-49](scenarios/capabilities.md#rolling-upgrade-proof-b-49) · [upgrade-coordinator](decisions/upgrade-coordinator.md) | [`rolling_upgrade_proof.rs`](../crates/trembita/tests/rolling_upgrade_proof.rs) (`b49_*`), [`upgrade_coordinator.rs`](../crates/trembita/src/integration/upgrade_coordinator.rs) |
| **B-48** | **Backup / restore / DR** — runbook, [`scripts/backup-data-dir.sh`](../scripts/backup-data-dir.sh), [deploy/backup-restore.md](../deploy/backup-restore.md), doctor preflight | [backup-restore § B-48](ops/backup-restore.md) · [capabilities § B-48](scenarios/capabilities.md#backup-restore-dr-b-48) · [runbook § B-48](ops/production-runbook.md#backup-restore-dr-b-48) | [`deploy_pack.rs`](../crates/trembita/tests/deploy_pack.rs) (`b48_*`), [`backup.rs`](../crates/trembita-tools/src/ops/backup.rs) (`b48_*`) |
| **B-47** | **OAuth prod hardening** — PKCE S256, redirect allowlist, [`DevOidcAuthorize`](../crates/trembita-gateway-auth/src/authorize.rs), hardened [`examples/oauth-gateway`](../examples/oauth-gateway/) | [gateway-cluster-auth § B-47](decisions/gateway-cluster-auth.md#b-47--oauth-prod-hardening) · [capabilities § B-47](scenarios/capabilities.md#oauth-prod-hardening-b-47) · [runbook § B-47](ops/production-runbook.md#oauth-production-wiring-b-47) | [`trembita-gateway-auth`](../crates/trembita-gateway-auth/) (`b47_*`) |
| **B-46** | **Gateway session Postgres adapter** — [`trembita-gateway-session-postgres`](../crates/trembita-gateway-session-postgres/); `gateway-session-postgres` feature; [`pg_gateway_session_gate`](../crates/trembita/src/gateway/pg_gateway_session.rs) + B-40 issuer/verifier ports | [gateway-cluster-auth § B-46](decisions/gateway-cluster-auth.md#b-46--postgres-gateway-session-store) · [capabilities § B-46](scenarios/capabilities.md#gateway-session-store-adapters-b-46) | [`trembita-gateway-session-postgres`](../crates/trembita-gateway-session-postgres/src/lib.rs) (`b46_*`), [`pg_gateway_session.rs`](../crates/trembita/src/gateway/pg_gateway_session.rs) |
| **B-45** | **Production deploy pack** — repo [`deploy/`](../deploy/README.md): systemd unit template, seed/joiner env + matrix, nginx LB example, rolling upgrade on systemd; runbook § B-45 | [production-runbook § B-45](ops/production-runbook.md#production-deploy-pack-b-45) · [capabilities § B-45](scenarios/capabilities.md#production-deploy-pack-b-45) · [ingress-lb](ops/ingress-lb.md) | [`deploy/`](../deploy/) (`b45_*` in [`deploy_pack.rs`](../crates/trembita/tests/deploy_pack.rs)) |
| **B-44** | **Coordination closed-loop** — hard ceilings (`TREMBITA_COORDINATION_MAX_*`, [`TrembitaConfigure`](../crates/trembita/src/configure.rs)); leader auto-shard registry + `why_not_scaling`; [`add_raft_groups`](../crates/trembita/src/app/runtime.rs) rejects at Raft ceiling; **`coordination_closed_loop`** on ops-summary | [runbook § B-44](ops/production-runbook.md#coordination-closed-loop-b-44) · [capabilities § B-44](scenarios/capabilities.md#coordination-closed-loop-b-44) · [env § B-44](env.md#trembita_coordination_max_queue_shards) | [`coordination_closed_loop.rs`](../crates/trembita-jobs/src/coordination_closed_loop.rs) (`b44_*`), [`queue_auto_shard.rs`](../crates/trembita-jobs/src/queue_auto_shard.rs), [`product_coordination_scale.rs`](../crates/trembita/tests/product_coordination_scale.rs) (`b44_*`), [`ops_summary_report.rs`](../crates/trembita/tests/ops_summary_report.rs) (`b44_*`) |
| **B-43** | **Ops cockpit (introspect)** — **`GET /introspect/ops-summary`**: join ([B-35](decisions/cluster-elasticity.md#join-readiness-pipeline-b-35)), product scale ([B-33](decisions/capability-dx.md#group-scale-b-28)), directory R3 ([B-36](decisions/actor-routing.md#r3-visibility--sticky-recovery-b-36)), coordination preset ([B-37](decisions/capability-dx.md#coordination-growth-presets-b-37)), queue depth hints; nested JSON ≡ dedicated `/introspect/*` routes | [runbook § B-43](ops/production-runbook.md#ops-cockpit-introspect-b-43) · [capabilities § B-43](scenarios/capabilities.md#ops-cockpit-introspect-b-43) · [capability-dx § B-43](decisions/capability-dx.md) | [`ops_summary.rs`](../crates/trembita/src/app/ops_summary.rs), [`gateway.rs`](../crates/trembita/src/app/gateway.rs) (`b43_*` — 13 tests: [regression table](scenarios/capabilities.md#automated-regression-b-43)) |
...
Full test commands: [testing-coverage § B-28–B-32](testing-coverage.md#shipped-backlog-b-28b32) · [§ B-33–B-41](testing-coverage.md#shipped-backlog-b-33b41) · [§ B-42+](testing-coverage.md#shipped-backlog-b-42). Planning snapshots: [archive B-28–B-32](archive/backlog-wave-b28-b32.md) · [archive B-33–B-41](archive/backlog-wave-b33-b41.md).
...
| [backlog.md](backlog.md) | Open work (empty epic table; deferred ADR topics) |
| [status § Product scale wave](#product-scale-wave-b-28b32) | Shipped **B-28 … B-54** epic index |
| [archive/backlog-wave-b28-b32.md](archive/backlog-wave-b28-b32.md) · [archive/backlog-wave-b33-b41.md](archive/backlog-wave-b33-b41.md) | Planning snapshots |
| [testing-coverage.md](testing-coverage.md#shipped-backlog-b-28b32) | Per-epic regression commands |
