...
Implementation backlog derived from the decision records in `docs/decisions/`. Organized by **epics**, with a **dependency graph** and **parallel tracks** so independent work can proceed simultaneously.
...
| W0.2 | CI: fmt, clippy -D warnings, test, MSRV, (publish dry-run at release) ([library-and-publishing](decisions/library-and-publishing.md)) | M | W0.1 | ✅ done (fast + nightly lanes) |
| W0.3 | `rustfmt.toml`, `clippy.toml`, license files (MIT/Apache) | S | W0.1 | ✅ done |
| W0.4 | `craft-proto`: `NodeId/Term/LogIndex`, `LogEntry`, RPC enums, client + join + actor wire types, `postcard` codec ([serialization](decisions/serialization.md)) | L | W0.1 | ✅ done (+ roundtrip tests) |
...
> **Track D progress (D0):** the application state-machine port from state-machine now lives in `craft-core`: the `StateMachine` trait (`apply`/`query`/`snapshot`/`restore` with associated `Command`/`Query`/`Response`/`Error` types) plus `Command` and `Query` marker traits. The "encode/decode glue" state-machine wanted from a derive is instead provided by **blanket impls** over any `serde` type that also meets the replication bounds — `Command: Clone + Send + 'static`, `Query: Send + 'static` — so a borrowed or non-`Clone` command simply fails to compile (the exact owned/clone-safe check state-machine specified), and **D1's bespoke derive is unnecessary**. A reference in-memory KV machine and 10 tests cover set/delete/append (incl. the error path leaving state untouched), the applied-index watermark, snapshot↔restore replacement, malformed-snapshot rejection, determinism (byte-identical snapshots for identical input), and the `Command`/`Query` codec round-trips.
...
| E4 | Client handling + **transparent forward** ([client-routing](decisions/client-routing.md)) | M | **done** — client propose/query correlated to responses; a follower transparently forwards `/client/wire` requests to the leader over the transport (bounded by `forward_timeout`) and returns its response; `Error` when no leader is known |
| E5 | Join handling → membership change ([join-rpc](decisions/join-rpc.md), [join-version-skew](decisions/join-version-skew.md)) | L | **done** — `/cluster/join` → leader runs joint-consensus membership change; version-skew hard-reject (join-version-skew), duplicate/joins-disabled rejects, follower forwards to leader, `Accepted` on commit. `allow_join` gates acceptance |
| E6 | `ActorRegistry`: local spawn/spawn_pool/scale_local/stop ([elastic-cluster](decisions/elastic-cluster.md)) | L | **done** — local `UserActor` runtime: `spawn`/`spawn_pool`/`scale_local`/`stop`, `ActorRef`/`PoolRef` with RR + keyed routing and `ask` via `RpcReplyPort`; production one-worker-per-name guard (one-worker-per-vps). tokio-based (not `ractor`), matching E1 |
| E7 | Cross-node directory + `resolve`/`cluster` ([cross-node-actors](decisions/cross-node-actors.md)) | L | **done** — `ActorDirectory` merged cluster view (per-node authoritative, monotonic-`epoch` state-based LWW; empty snapshot revokes), `resolve(ActorId)`/`lookup`/`cluster(name)→ClusterRef` with RR + keyed target selection; `DirectorySync` publishes the local snapshot over `/actor/register` and serves inbound as a `RequestHandler`. Proto: `ActorId`/`ActorTypeId`/`ActorRegistration`/`DirectoryUpdate`/`RegisterAck` + `send_directory_update`; `UserActor::MIGRATABLE`; `ActorRegistry::local_registrations`. Cross-node *delivery* is E8 |
| E8 | Remote delivery `/actor/deliver`, routing RR + keyed ([cluster-routing](decisions/cluster-routing.md)) | M | **done** — `ClusterMessaging.cast`/`cast_keyed` resolve a target instance via the directory (E7) then deliver locally (`ActorRegistry::deliver_local`) or ship an `ActorEnvelope` over `/actor/deliver`; serves inbound as a `RequestHandler` → `DeliverAck`. Wire ingress is type-erased via `UserActor::decode_message` (default = local-only `NotAddressable` opt-out); proto `ActorEnvelope{to,from,req_id,payload,reply_expected}` + `DeliverAck{delivered,error,reply}` + `send_actor_deliver`. **Cross-node `ask`** (request/reply) now ships too: `ClusterMessaging.ask`/`ask_keyed` send an envelope with `reply_expected`, the receiver decodes via `UserActor::decode_ask` (default `NotAddressable`) with a `WireReplyPort` that `postcard`-encodes whatever the handler replies through the actor's normal `RpcReplyPort`, and the reply rides back in `DeliverAck.reply` (server-side wait bounded by a 30s deadline). 2-node LocalNetwork tests for RR local+remote split, keyed pinning, no-target, remote-missing-instance, non-addressable rejection, plus ask round-trip / ask-to-non-addressable / ask-no-target |
| E9 | `spawn_remote` / `scale_cluster` + placement ([cross-node-actors](decisions/cross-node-actors.md)) | L | **done** — `ClusterControl.spawn_remote` (local, or `SpawnRequest` over `/actor/spawn` reconstructed by a per-node type factory via `register_type::<A>()`) and `scale_cluster` driving a group to a cluster-wide count. Pure `plan_scale` planner implements the one-worker-per-node model (one-worker-per-vps): stable keep/spawn/remove diff vs directory + live membership, `InsufficientNodes` when `total > nodes`. `scale_cluster` executes spawns and applies local removals; remote teardown returned in the plan for the supervisor (E10). Proto `SpawnRequest`/`SpawnReply` + `send_actor_spawn`; `UserActor::encode_config`/`decode_config` (default `NotSpawnable`). 13 tests: planner (fill/keep/cap/scale-down/dead-node/prune) + wire spawn (self/remote/unknown-type/not-spawnable) + scale (place/reject/demote) |
| E10 | **ClusterSupervisor** (leader-only) reconcile ([supervisor-leader](decisions/supervisor-leader.md)) | L | **done** — `ClusterSupervisor` holds a declarative set of managed groups (`manage::<A>(name, total, config)`) and a `reconcile()` that runs **only on the leader** (skips with `ran_as_leader=false` otherwise), diffing desired vs directory+membership via the E9 planner and executing spawns through `ClusterControl`. Idempotent once the directory converges; per-group errors surfaced without aborting the pass. Leadership/membership abstracted behind the `ClusterState` port (runtime wires the real one; mock for tests). `manage` registers the type factory so any node can place the group. 4 tests: follower-skip, one-per-node placement, idempotency, over-capacity error. **Non-leader `scale_cluster` forwarding** now lands too: `CraftCluster::scale_cluster::<A>` runs directly on the leader (planning against the current voter set) and, on a follower, transparently forwards a `ScaleRequest` (name/type/total/encoded-config + observed committed voters) to the leader over `/actor/scale` (client-routing-style), which replays it via `ClusterControl::scale_cluster_encoded`/`handle_scale` using the requester's voter view; `send_actor_scale` + facade `ScaleClusterError`. 1 facade test: a follower scales a group and one worker lands on every node |
| E11 | Auto-spawn workers on join ([auto-spawn-on-join](decisions/auto-spawn-on-join.md)) | M | **done** — `ClusterSupervisor::manage_auto::<A>(name, config)` declares an auto-worker group whose target **tracks the live membership** (one per node): a node that joins gets a worker on the next leader reconcile, a node that leaves has its instance planned for removal. Spawns are now **idempotent** end-to-end (local `NameExists` → no-op; remote `SpawnReply` treats `NameExists` as success, cross-node-actors), so reconcile is safe even before the directory converges. `Target::{Fixed,PerLiveNode}` resolved per pass; blanket `ClusterState for Arc<T>`. 3 added tests: auto-worker on joiner, idempotent reconcile w/o directory, departed-node removal plan. **Reconcile-on-membership-commit is now wired**: the facade's facts-refresher loop diffs successive `NodeStatus` and triggers `supervisor.reconcile()` immediately on any committed voter-set change (join *or* leave), so joiners get workers without waiting for the periodic timer (leader-gated inside `reconcile`) |
| E12 | Migration on leave/crash + drain timeout ([drain-timeout](decisions/drain-timeout.md)) | L | **done** — **Graceful drain** (drain-timeout): actor instances run a serial mailbox that also carries a snapshot-capture control item, so `ActorRegistry::stop_graceful(name, timeout)` sets a `draining` flag (new sends rejected with `SendError::Draining`/`DeliverError::Draining`), lets queued + in-flight work finish, and force-aborts on timeout, returning `DrainOutcome::{Completed,TimedOut}`. `DEFAULT_DRAIN_TIMEOUT = 60s`. **Migration** (cross-node-actors): `UserActor::{migration_snapshot,restore_migration}` (stateless defaults: empty snapshot / no-op); `ClusterControl::migrate::<A>(from, to_node, config, drain_timeout)` captures the source's snapshot **via the mailbox** (ordered after queued messages), ships `MigrateRequest` over `/actor/migrate`, and the target factory spawns a replacement via `spawn_restoring` (restore before first message), then the source is drained+stopped; generation bumped. Idempotent target (`NameExists` → success). 5 tests: drain completes / times-out-and-rejects, stateful state transfer + source teardown, non-local + same-node rejection. **Leave/crash → auto-heal is now wired into the runtime**: the facts-refresher loop diffs committed voters; when a node departs, every node prunes it from its `ActorDirectory` (`remove_node`, so routing stops targeting it) and the leader's `reconcile()` relocates the departed node's managed/auto workers onto survivors. (True *stateful* migration requires the source to be alive and is initiated by the leaving node via `ClusterControl::migrate`; a crashed node's in-memory state is unrecoverable, so its managed workers respawn fresh, per cross-node-actors, drain-timeout.) |
| E13 | 1-worker-per-VPS enforcement + dev mode ([one-worker-per-vps](decisions/one-worker-per-vps.md)) | S | **done** — production is the default (one worker per node per name); `spawn_pool`/`scale_local` above 1 are rejected with `{Spawn,Scale}Error::MultiWorkerDisabled` (ADR-named), a repeat `spawn` is `NameExists`, and `scale_cluster` caps at one-per-live-node via the E9 planner. `ActorRegistry::{new,new_dev}` + the documented `placement_mode() -> PlacementMode::{Production,DevelopmentMulti}` API. Added the **resource** value objects `ResourceProfile::{UseAllAvailable,Limited}` (default all) and `VpsResources::{detect,from_parallelism}` (parallelism lives *inside* the one worker, one-worker-per-vps) with 5 unit tests; registry gains 1 placement-mode test. Facade `--dev-multi-workers` / `RAFT_DEV_MULTI_WORKERS` env plumbing + startup warning land with the `craft` builder (Wave 4) |
| E14 | Restart policies ([observability](decisions/observability.md) §5) | M | **done** — OTP-style `RestartPolicy::{Never,OnFailure{max_restarts,window},Always}` (default `Never`). A `handle` error is craft's *failure* signal: `Never` keeps state (non-fatal, current default), `Always` rebuilds fresh state via `start`, `OnFailure` restarts within a sliding-window budget then **escalates** (instance stops + deregisters). `ActorRegistry::spawn_supervised::<A>(name, cfg, policy)` (supervised `Config: Clone`); `ActorRef::restart_count()` exposes the running tally. Rebuild is a per-task `Box<dyn Fn>` (Send, no Sync); the roster moved behind its own `Arc` so an escalating task can self-deregister without keeping the pool alive. 3 tests: never-keeps-state, always-resets, on-failure budget-then-escalate. **Restart telemetry** now surfaces (Track H): the registry exposes an `ActorObserver` hook (`set_observer`) fired on spawn / restart / escalation / stop and per message, kept telemetry-agnostic; the facade installs an `ActorTelemetry` bridge that bumps `craft_actor_restarts_total`/`craft_actor_escalations_total` metrics and emits `CraftEvent::ActorRestarted{count}` / `ActorStopped{reason: RestartLimit}` (see the Track H "runtime → telemetry wiring" note for the full lifecycle/message/consensus surface). +1 registry test asserting the observer sees the restart then the escalation |

> **Note (E5 dynamic membership / [discovery](decisions/discovery.md)):** membership
> in the Raft log carries only `NodeId`s, so a companion **address plane** now
> lets a node join knowing just one seed address instead of a static,
> cluster-wide peer map. `QuicTransport`'s peer directory became runtime-mutable
> (`learn_peer`/`forget_peer`/`peers`, guarded by an `RwLock` never held across
> an `.await`); a new `/cluster/peers` route (`Route::ClusterPeers`, proto
> `PeerBook`/`PeerEntry`, `fetch_peers`) serves a node's known addresses. The
> facade's `NodeRouter` records the joiner's `advertise_addr` off the
> `/cluster/join` body (so the leader — reached directly or via a forwarding
> follower — can dial the newcomer) and answers `/cluster/peers`; a per-node
> anti-entropy loop pulls each known peer's book and merges it, so addresses
> converge in both directions. `CraftClusterBuilder::join(seed_id, addr)` drives
> the handshake on `start_quic`: fetch the seed's book, then submit the join
> (retried until it commits). The joiner is configured with the *current* voter
> set (not itself), so — thanks to Pre-Vote's non-voter guard — it never disrupts
> the live leader while catching up, and becomes a voter once the `ConfChange`
> commits. Covered by a live-QUIC test (4th node joins a running 3-node cluster
> from a seed, becomes a voter, replicates) and the `vps_join` example.
...
| F2 | `RemoteClient` (HTTP/3 + client mTLS) ([security](decisions/security.md)) | M | done |
...
followers already forward server-side (client-routing), so the client only needs one
...
| G2 | Redis impl (`fred`/`redis`) get/set/del/CAS/TTL ([actor-state-redis](decisions/actor-state-redis.md)) | M | done |
...
| ID | Task | Effort | Status |
|----|------|--------|--------|
| H1 | Admin HTTP `:8080`: `/health`, `/ready` ([health-admin-port](decisions/health-admin-port.md)) | M | done |
| H2 | Prometheus `/metrics` ([observability](decisions/observability.md)) | M | done |
...
| H6 | Opt-in message tracing | M | **done**¹ |

**Track H notes.** `craft-dashboard` provides the full read-only admin surface
on the separate admin port (health-admin-port), built on hyper HTTP/1.1:
...
- **H5** — a single self-contained embedded `/dashboard` HTML page (cluster
  ...
  fed from the `EventBus`); read-only per observability §6.
...
**Runtime → telemetry wiring (Track H completion).** The dashboard/metrics/event
surfaces are now fed by the runtime, not just the restart hook:

- **Actor lifecycle + per-message telemetry** — `craft-actor` generalised the
  supervision hook into `ActorObserver` (`on_spawned`/`on_stopped`/`on_restart`/
  `on_escalated`/`on_message_handled`), bound once per instance task at launch
  (no per-message locking). Each instance task keeps cheap relaxed atomics
  (messages handled, cumulative handle-nanos, mailbox depth) exposed via
  `ActorRegistry::stats() -> Vec<ActorGroupStats>`. The facade's `ActorTelemetry`
  bridge emits `ActorSpawned`/`ActorStopped`/`ActorRestarted` events + counters,
  and a **metrics sampler** loop turns the registry counters into
  `craft_actor_{instances,mailbox_depth,messages_total,handle_seconds_total}`
  series + `MailboxDepth` events (differencing monotonic counters per interval).
- **Consensus + membership telemetry** — the facts-refresher loop now diffs
  successive `NodeStatus` via `MembershipTelemetry`: it publishes
  `craft_raft_{term,commit_index,last_applied,live_nodes,is_leader}` gauges and,
  on transitions, emits `LeaderChanged`/`NodeJoined`/`NodeLeft` (+ the
  `*_total` counters).
- ¹ **H6** — opt-in per-message tracing: `CraftCluster::trace(name, TraceOpts)`
  enables auto-expiring per-group tracing; while active each handled message
  emits `CraftEvent::MessageHandled` (with latency), gated behind a fast
  `AtomicBool` so untraced steady-state flow stays lock-free. Tested end-to-end
  in the facade (`telemetry_publishes_consensus_and_actor_metrics`,
  `opt_in_tracing_emits_message_handled_events`) + the `ActorObserver`
  restart/escalation unit test.
...
| I5 | Linearizability checker (`porcupine`-style) over sim histories ([testing-strategy](decisions/testing-strategy.md)) | L | ✅ done |
...
#### Track T — Testing & CI (cross-cutting) **[P]** ([testing-strategy](decisions/testing-strategy.md), [coverage matrix](testing-coverage.md))
| ID | Task | Effort | Notes |
|----|------|--------|-------|
...
| T4 | `craft-fuzz`: `postcard`/wire decoders (`cargo-fuzz`) | M | **done** — `crates/craft-fuzz/fuzz/wire_decode` (RaftRpc, ClientRequest, JoinRequest, LeaveRequest, GroupPeerEnvelope, GroupMigrateRequest, ActorEnvelope); nightly CI `fuzz` job |
...
| J2 | Builder options: node_id, members, raft_config, tick_period, allow_join, dev_multi_workers, forward_timeout, reconcile/publish periods, event_capacity, admin_addr, register_actor / manage / manage_auto | M | **done** — fluent, all `#[must_use]`. `manage_auto::<A>` wires auto-spawn-on-join auto-workers into the reconcile loop; `admin_addr` spawns the health-admin-port `AdminServer` over a `CraftObserver<M>` (readiness/cluster/actors/node views from the live handle + directory). A bad admin bind is logged, not fatal. `dev_multi_workers` selects `ActorRegistry::new_dev`. 3 facade E2E tests over `LocalNetwork`: elect+propose/query, auto-place a worker per node, and admin endpoints reporting live cluster/actor state. `resource_profile`/`actor_state_store` builder knobs + `listen`/`join` land with J3 (QUIC) |
| J3 | `craft-node` reference binary + CLI/env + facade `start_quic` (QUIC transport, mTLS) | M | **done** — `CraftClusterBuilder::start_quic(security, listen, peers)` is the production path: builds mTLS server/client configs (security), binds a `QuicServer`, and **shares the bound endpoint** so outbound dials reuse the listener socket, then drives the node over a `QuicTransport` (accept loop tracked for shutdown). New `Security` type (`new` / `from_ca_certs` / feature-gated `dev` via `ClusterCa`) + `StartError`; `PeerDirectory` re-exported. `craft-node` reads `CRAFT_NODE_ID`/`CRAFT_LISTEN`/`CRAFT_ADMIN`/`CRAFT_PEERS` + PEM cert vars (`CRAFT_CERT`/`KEY`/`CA`), falling back to a throwaway dev CA for a **single-node** cluster (multi-node requires shared certs, errors clearly otherwise); runs a built-in demo state machine until Ctrl-C. Tested: a real 3-node QUIC/mTLS cluster (shared dev CA, one UDP socket each) elects a leader and replicates propose/query over HTTP/3; the binary self-elects and serves admin. **Dynamic `JOIN_ADDR` membership** (peer-address propagation) now lands — see the E5/discovery note below and the `vps_join` example (J4) |
| J4 | Examples: KV store, three-node local, cluster actors, Redis worker | L | **done** — three runnable `craft` examples (all assert + exit 0): `kv_store` (single-node facade, propose/query/delete via the in-process handle), `three_node_local` (3 nodes over `LocalNetwork` driven by a `RemoteClient`/`TypedClient` that hits any node and is transparently forwarded to the leader, client-routing), and `actors_cluster` (a `#[remote_actor]` `Worker` auto-placed one-per-node by the leader, then 6 jobs cast round-robin cluster-wide exercising local **and** cross-node delivery). The Redis worker example already ships as `craft-store-redis/examples/idempotent_worker` (Track G). **`vps_join`** (feature `dev-certs`) now demonstrates elastic membership: a 4th node joins a running 3-node QUIC/mTLS cluster knowing only a seed address, becomes a voter, and catches up (discovery, join-rpc) |
| J5 | `docs.rs` metadata, README quickstart, CHANGELOG | M | **done** — `[package.metadata.docs.rs] all-features = true` on every library crate (incl. the proc-macro crate) so docs.rs builds the full API. Root `README.md` (badges, why-craft, install, crate map, examples, `e2e/`, MSRV, dual-license) + a focused crate-level `crates/craft/README.md` wired via `readme = "README.md"` for the crates.io page. `CHANGELOG.md` in Keep-a-Changelog format with an `Unreleased`/`0.1.0` section cataloguing the whole workspace. `LICENSE-MIT`/`LICENSE-APACHE` already present. Verified: `cargo metadata` parses and `RUSTDOCFLAGS=-D warnings cargo doc --workspace --no-deps --all-features` is clean. `cargo publish --dry-run` wiring rolls with J7 (needs registry-published path deps or release tooling) |
| J6 | `examples/certs/` script + `docs/certs.md` ([cert-provisioning](decisions/cert-provisioning.md)) | M | **done** — `examples/certs/generate.sh` mints a cluster CA, per-node certs (CN **and** DNS SAN = `craft-node-<id>`, the SNI craft dials + `serverAuth,clientAuth` EKU since a node is both), and client certs (`clientAuth`), using only `openssl` so it runs on OpenSSL **and** LibreSSL (P-256 keys, `chmod 600`). `docs/certs.md` covers the script + the by-hand OpenSSL equivalent, the env-var reference, embedding via `Security::from_ca_certs`, joining a VPS, rotation (rolling restart; CA bundle swap), the dev/insecure single-node path, and a Redis-TLS pointer. Aligned `craft-node` to the cert-provisioning names (`CRAFT_NODE_CERT`/`CRAFT_NODE_KEY`/`CRAFT_CA_CERT`). **Validated end-to-end:** two separate `craft-node` processes using script-generated certs formed a real QUIC/mTLS cluster (node 1 leader, node 2 follower, agreeing views) — unblocks the T6/T8 docker-compose E2E |
| J7 | Release tooling (`cargo release`, publish order) ([library-and-publishing](decisions/library-and-publishing.md)) | M | **done** — native `cargo publish --workspace` (cargo 1.90+) resolves the intra-workspace path deps locally and uploads in dependency order (`craft-macros,craft-proto → craft-core,craft-storage,craft-net → craft-actor → craft-client,craft-dashboard,craft-sim,craft-store-redis → craft → craft-node`), so no external `cargo-release` is needed. `scripts/release.sh <version> [--publish]` bumps the single `[workspace.package]` version **and** the internal `craft* = { path, version }` pins (awk, leaving `rust-version` alone), refreshes `Cargo.lock`, runs `cargo publish --workspace --dry-run` as the gate, then commits `chore(release): craft vX.Y.Z` + annotated tag; `--dry-run` alone just runs the gate. `.gitlab-ci.yml` adds a `release` stage: `publish-dry-run` (on `v*` tags + schedule) and a **manual** `publish` (on `v*` tags, needs `CARGO_REGISTRY_TOKEN`). Runbook in `docs/releasing.md`. Verified: `cargo publish --workspace --dry-run` packages all 12 crates; the bump awk + shellcheck are clean |
...
Testing (Track T) is **cross-cutting**: each stream writes tests for its own code; the Consensus stream owns the sim harness + linearizability checker; DX owns CI lanes + E2E/chaos infra. See [testing-strategy](decisions/testing-strategy.md).
...
## Post-v1 increments (future-work-and-risks deferrals)

Work on the intentionally-deferred items from [future-work-and-risks](decisions/future-work-and-risks.md).

| # | Item | Status | Where |
|---|------|--------|-------|
| P1 | **Lease reads** ([read-consistency](decisions/read-consistency.md)) | **done** | `RaftNode::lease_read` in `craft-core` + driver `query` fast-path. A quorum-confirmed lease (`election_timeout_min/2` ticks, measured from heartbeat broadcast, surrendered on step-down) serves `query` with no ReadIndex round-trip once the read index is applied; otherwise falls back to ReadIndex. 7 core lease tests + existing read tests. **Follower reads** (`ReadIndexConfirm` + local query on non-leaders) landed in `craft-actor` runtime (`route_query`, `follower_query_bytes`). |
| P2 | **Gossip discovery** ([discovery](decisions/discovery.md)) | **done** | `craft::discovery` (`Seed`, `dedupe_seeds`, `resolve_dns_seeds`) + `CraftClusterBuilder::join_seeds`; `join_cluster` rotates through a resilient seed set for both peer-book pull and join. Membership stays Raft-authoritative; peer addresses still converge via `/cluster/peers`. |
| P3 | **Dev-only JSON wire** | **done** | `craft-proto/json-wire` cargo feature swaps the shared `encode`/`decode` from `postcard` to `serde_json` (zero call-site changes); `WIRE_CODEC` reports the active format, logged at node startup. Forwarded as `craft/json-wire`. |
| P4 | **QUIC traffic-priority tuning** (future-work-and-risks R2) | **done** | `craft_net::{TrafficPolicy,RateLimiter}` — per-`TrafficClass` token-bucket admission control in the QUIC `round_trip`, so bulk client/actor traffic can't starve consensus. `QuicTransport::with_policy` + `CraftClusterBuilder::traffic_policy`; defaults unlimited (`Peer` never throttled). |
| P5 | **K8s / cloud integration** | **done** | `deploy/Dockerfile` (distroless release image) + `deploy/kubernetes/` (StatefulSet, headless + client Services) wired to `/health` `/ready` probes; `craft-node` derives its id from the pod ordinal (`POD_NAME`) and supports `CRAFT_JOIN_SEEDS` / `CRAFT_DISCOVERY` (DNS) / `CRAFT_ALLOW_JOIN`. |
| P6 | **Write sharding / multi-Raft** ([write-sharding-multi-raft](decisions/write-sharding-multi-raft.md)) | **done** | Routing, runtime, builder, keyed client APIs, per-group `GroupRedbLayout` / `data_dir`, leader rebalance control plane (`RaftGroupReconciler`, `MultiRaftState::rebalance`), cross-node group migration RPC (`/cluster/group/migrate`). |
| P7 | **Liveness ≠ membership** ([liveness-vs-membership](decisions/liveness-vs-membership.md)) | **done** | Leader-side failure detector in `craft-core`: per-peer last-ack tick → `RaftNode::reachable(window)`/`reachable_now()` (default `2×election_timeout_max`), surfaced as `NodeStatus.reachable` and `ClusterState::reachable_nodes()` (defaults to `live_nodes()`), populated on `ClusterFacts`. 5 core reachability tests. **Crash-driven reconcile landed:** `ClusterSupervisor` plans placement against `reachable_nodes()` (auto-worker count tracks reachable nodes; fixed groups cap at reachable set); facts-refresher triggers reconcile + directory prune on reachability changes. 2 supervisor + 2 telemetry unit tests. |
| P8 | **Per-group Raft membership** ([per-group-raft-membership](decisions/per-group-raft-membership.md)) | **done** | Pure planner + runtime: `group_voters`, `plan_group_membership_sync`, `NodeHandle::propose_membership`, `MultiRaftState::sync_group_membership` on membership delta, `CraftClusterBuilder::group_replication_factor`. Group 0 = cluster join; groups 1+ sync via per-group leaders. Integration test `join_syncs_non_coordinator_group_membership`. |
| P9 | **mTLS cert automation** ([cert-automation](decisions/cert-automation.md)) | **done** | Hot reload: `craft-net` PEM load + `QuicServer`/`QuicTransport` config swap; facade `PemSecurity`, `CertReloadHandle`, `start_quic_pem`, `cert_watch`; `craft-node` polls + `SIGHUP`. Ops: `examples/step-ca/`, `deploy/kubernetes/cert-manager/`, `docs/certs.md` § Automation. Tests: `craft/tests/cert_reload.rs` (loopback), `e2e/cert_renew.sh` (docker-compose SIGHUP + poll). |
| P10 | **Tier 2 production reliability** ([tier2-production-reliability](decisions/tier2-production-reliability.md)) | **done** | Reachability tuning + phi-accrual (`ReachabilityConfig`, builder API). Rolling wire N/N−1 (`protocol_version_compatible`). Admin TLS (`serve_tls`, `CRAFT_ADMIN_TLS_*`). `craft-ops` backup/restore CLI. `e2e/linearizability.sh` + scheduled CI (`linearizability-sim`, docker phase in `e2e`). |
| P11 | **Tier 1 multi-Raft advances** ([tier1-multi-raft-advances](decisions/tier1-multi-raft-advances.md)) | **done** | Per-group learners (`group_learner_factor`), operator shard expansion (`expand_shard_count`), non-atomic keyed batch (`propose_keyed_batch`), `GET /introspect/raft-groups`. Integration test `tier1_shard_expansion_and_keyed_batch`. |
| P12 | **Tier 2 multi-Raft architecture** ([tier2-multi-raft-architecture](decisions/tier2-multi-raft-architecture.md)) | **partial** | Phase 1–4 landed (planners, dynamic catalog, stable shard wiring, saga coordinator). Optional 2PC deferred per [cross-shard-transactions](decisions/cross-shard-transactions.md). |

## Related

- [architecture.md](architecture.md) · [README.md](README.md) (decision index) · [open-questions.md](open-questions.md)
