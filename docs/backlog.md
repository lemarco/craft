...
> **Note (E5 dynamic membership / [ADR 007](decisions/007-discovery.md)):** membership
...
#### Track T — Testing & CI (cross-cutting) **[P]** ([ADR 029](decisions/029-testing-strategy.md), [coverage matrix](testing-coverage.md))
| ID | Task | Effort | Notes |
|----|------|--------|-------|
...
| T4 | `craft-fuzz`: `postcard`/wire decoders (`cargo-fuzz`) | M | **done** — `crates/craft-fuzz/fuzz/wire_decode` (RaftRpc, ClientRequest, JoinRequest, GroupPeerEnvelope, GroupMigrateRequest, ActorEnvelope); nightly CI `fuzz` job |
...
## Post-v1 increments (ADR 027 deferrals)

Work on the intentionally-deferred items from [ADR 027](decisions/027-future-work-and-risks.md).

| # | Item | Status | Where |
|---|------|--------|-------|
| P1 | **Lease reads** ([ADR 005](decisions/005-read-consistency.md)) | **done** | `RaftNode::lease_read` in `craft-core` + driver `query` fast-path. A quorum-confirmed lease (`election_timeout_min/2` ticks, measured from heartbeat broadcast, surrendered on step-down) serves `query` with no ReadIndex round-trip once the read index is applied; otherwise falls back to ReadIndex. 7 core lease tests + existing read tests. **Follower reads** (`ReadIndexConfirm` + local query on non-leaders) landed in `craft-actor` runtime (`route_query`, `follower_query_bytes`). |
| P2 | **Gossip discovery** ([ADR 007](decisions/007-discovery.md)) | **done** | `craft::discovery` (`Seed`, `dedupe_seeds`, `resolve_dns_seeds`) + `CraftClusterBuilder::join_seeds`; `join_cluster` rotates through a resilient seed set for both peer-book pull and join. Membership stays Raft-authoritative; peer addresses still converge via `/cluster/peers`. |
...
| P4 | **QUIC traffic-priority tuning** (ADR 027 R2) | **done** | `craft_net::{TrafficPolicy,RateLimiter}` — per-`TrafficClass` token-bucket admission control in the QUIC `round_trip`, so bulk client/actor traffic can't starve consensus. `QuicTransport::with_policy` + `CraftClusterBuilder::traffic_policy`; defaults unlimited (`Peer` never throttled). |
...
| P6 | **Write sharding / multi-Raft** ([ADR 031](decisions/031-write-sharding-multi-raft.md)) | **done** | Routing, runtime, builder, keyed client APIs, per-group `GroupRedbLayout` / `data_dir`, leader rebalance control plane (`RaftGroupReconciler`, `MultiRaftState::rebalance`), cross-node group migration RPC (`/cluster/group/migrate`). |
| P7 | **Liveness ≠ membership** ([ADR 032](decisions/032-liveness-vs-membership.md)) | **done** | Leader-side failure detector in `craft-core`: per-peer last-ack tick → `RaftNode::reachable(window)`/`reachable_now()` (default `2×election_timeout_max`), surfaced as `NodeStatus.reachable` and `ClusterState::reachable_nodes()` (defaults to `live_nodes()`), populated on `ClusterFacts`. 5 core reachability tests. **Crash-driven reconcile landed:** `ClusterSupervisor` plans placement against `reachable_nodes()` (auto-worker count tracks reachable nodes; fixed groups cap at reachable set); facts-refresher triggers reconcile + directory prune on reachability changes. 2 supervisor + 2 telemetry unit tests. |
| P8 | **Per-group Raft membership** ([ADR 033](decisions/033-per-group-raft-membership.md)) | **done** | Pure planner + runtime: `group_voters`, `plan_group_membership_sync`, `NodeHandle::propose_membership`, `MultiRaftState::sync_group_membership` on membership delta, `CraftClusterBuilder::group_replication_factor`. Group 0 = cluster join; groups 1+ sync via per-group leaders. Integration test `join_syncs_non_coordinator_group_membership`. |
| P9 | **mTLS cert automation** ([ADR 034](decisions/034-cert-automation.md)) | **done** | Hot reload: `craft-net` PEM load + `QuicServer`/`QuicTransport` config swap; facade `PemSecurity`, `CertReloadHandle`, `start_quic_pem`, `cert_watch`; `craft-node` polls + `SIGHUP`. Ops: `examples/step-ca/`, `deploy/kubernetes/cert-manager/`, `docs/certs.md` § Automation. Tests: `craft/tests/cert_reload.rs` (loopback), `e2e/cert_renew.sh` (docker-compose SIGHUP + poll). |
