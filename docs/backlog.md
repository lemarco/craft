...
| P6 | **Write sharding / multi-Raft** ([ADR 031](decisions/031-write-sharding-multi-raft.md)) | **done** | Routing, runtime, builder, keyed client APIs, per-group `GroupRedbLayout` / `data_dir`, leader rebalance control plane (`RaftGroupReconciler`, `MultiRaftState::rebalance`). Deferred: cross-node group migration RPC. |
| P7 | **Liveness ≠ membership** ([ADR 032](decisions/032-liveness-vs-membership.md)) | foundation | Leader-side failure detector in `craft-core`: per-peer last-ack tick → `RaftNode::reachable(window)`/`reachable_now()` (default `2×election_timeout_max`), surfaced as `NodeStatus.reachable` and `ClusterState::reachable_nodes()` (defaults to `live_nodes()`), populated on `ClusterFacts`. 5 core reachability tests. Placement still targets committed voters; **consuming the signal to migrate/respawn a crashed host's workers is deferred** (now unblocked). |
