...
| P6 | **Write sharding / multi-Raft** ([ADR 031](decisions/031-write-sharding-multi-raft.md)) | foundation | Pure routing foundation in `craft-core::shard`: `ShardRouter` (stable FNV-1a key→shard) + rendezvous shard→group placement (`place_shard`/`shard_assignment`) with a tested minimal-churn property. Full multi-group runtime wiring (N drivers, per-group storage, shard-aware client routing, rebalancing control plane) is deferred per the ADR. |
