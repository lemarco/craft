...
| P6 | **Write sharding / multi-Raft** ([ADR 031](decisions/031-write-sharding-multi-raft.md)) | **runtime wired** | Routing foundation in `craft-core::shard` plus runtime wiring: `ShardedNodeService`, `spawn_multi_raft_node`, `GroupTransport`, keyed client wire types, `RemoteClient`/`TypedClient` keyed helpers, integration test. Deferred: `CraftClusterBuilder` multi-group start, per-group redb namespacing, rebalancing control plane. |
