...
| **Idempotency / step progress** (survive worker crash) | [`ActorStateStore`](../../crates/trembita-capstore/src/store.rs) | `get` / `set` / CAS | `actor-store.redb` (default) | "order-42 processed" marker |
