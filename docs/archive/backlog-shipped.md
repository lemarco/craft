...
| B-14b   | 1 | **`trembita init` v2** — [`templates/trembita-app/`](../../crates/trembita-cli/templates/trembita-app/): `JobOpts` + `#[consumer]` + `IdempotencyOpts::by_dedup_key` + `default_max_attempts(5)`; remove bare `TODO` stub ([B-06](#b-06--trembita-init-project-template)) | ✅ |
...
Moved to [Open work](../backlog.md#open-work). **O-05** (self-update coordinator) shipped — see [upgrade-coordinator](../decisions/upgrade-coordinator.md).
