...
Target **0.5.0** — unified HTTP listener; see [unified-listener](docs/decisions/unified-listener.md) and [env.md](docs/env.md).
...
- **Gateway routing v2** — native [`Gateway`](crates/trembita-http/src/gateway/mod.rs) /
  ...
  for parity tests; see [gateway-routing-v2](docs/decisions/gateway-routing-v2.md).
...
- **`HostRouter`**, **`MultiHostBuilder`**, `host_router.rs`, `trembita::axum` re-exports (see [gateway-routing-v2](docs/decisions/gateway-routing-v2.md)).
...
> [`RouteTable`](crates/trembita-http/src/routing/table.rs) ([gateway-routing-v2](docs/decisions/gateway-routing-v2.md)).
