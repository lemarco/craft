...
| B-42 | Local elastic parity (4th joiner + LB/cap smoke) | `dev/cluster.rs` (`b42_*` spawn/waves/ports), `dev/local_cluster.rs` (`b42_*`), `tests/dev.rs` (`b42_*`), `e2e_elastic/{gateway,cap}.rs` (`b42_*`), `examples/realtime`, `local-cluster.sh` | `./scripts/test-fast.sh -p trembita-cli --lib b42_` · `./scripts/test-fast.sh -p trembita-cli --test dev b42_` · `./scripts/test-fast.sh -p trembita-tools --lib b42_` · live: `./scripts/local-cluster.sh elastic-smoke` |
