...
name: crafty-testing
description: >-
  Add or extend tests in the crafty workspace following testing-strategy — choose unit,
  driver, sim, integration, or E2E layer; use LocalNetwork, run_contract,
  seeded crafty-sim; update docs/testing-coverage.md. Use when writing tests,
  fixing test gaps, adding regressions, or asking how to test crafty code.
---
...
|---------|-------|----------|
| Pure FSM / log / RNG | Unit | `crafty-core/src/` or `tests/` |
| Persist / recover / `take_persist` | Driver | `crafty-actor/tests/driver.rs` |
| Partition, drop, safety invariant | Sim | `crafty-sim/tests/` + seed |
...
Add behavior to `run_contract<S>()` in `crafty-storage/tests/storage.rs`; run against Memory **and** Redb.
...
Reuse KV fixtures from [`crafty-test-support`](../crates/crafty-test-support/src/kv.rs).
