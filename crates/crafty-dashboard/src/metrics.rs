...
//!
//! Supports the three shapes crafty needs: monotonic **counters** (request
...
mod tests {
    ...
    fn counters_accumulate_and_render() {
        let m = Metrics::new();
        m.incr("crafty_client_requests_total", "Client requests.", &[], 1.0);
        m.incr("crafty_client_requests_total", "Client requests.", &[], 2.0);
        let out = m.render();
        assert!(out.contains("# TYPE crafty_client_requests_total counter"));
        assert!(out.contains("crafty_client_requests_total 3"));
    }
    ...
    fn gauges_overwrite_and_labels_are_sorted() {
        ...
        m.set("crafty_raft_role", "Role.", &[("z", "1"), ("a", "2")], 1.0);
        let out = m.render();
        assert!(out.contains("crafty_raft_commit_index{node=\"2\"} 9"));
        // Labels rendered in sorted key order regardless of input order.
        assert!(out.contains("crafty_raft_role{a=\"2\",z=\"1\"} 1"));
    }

    #[test]
    fn summaries_emit_count_and_sum() {
        let m = Metrics::new();
        m.observe("crafty_handle_latency_seconds", "Handle latency.", &[], 0.5);
        m.observe("crafty_handle_latency_seconds", "Handle latency.", &[], 1.5);
        let out = m.render();
        assert!(out.contains("# TYPE crafty_handle_latency_seconds summary"));
        assert!(out.contains("crafty_handle_latency_seconds_count{} 2"));
        assert!(out.contains("crafty_handle_latency_seconds_sum{} 2"));
    }
}
