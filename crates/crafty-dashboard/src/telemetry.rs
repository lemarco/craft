...
//!
//! The runtime emits [`CraftyEvent`]s onto an [`EventBus`] (a bounded broadcast
...
/// Why an actor stopped (surfaced in [`CraftyEvent::ActorStopped`]).
...
pub struct TraceOpts {
    /// Emit a [`CraftyEvent::MessageTraced`] per handled message.
    pub messages: bool,
...
/// A bounded broadcast bus for [`CraftyEvent`]s.
///
...
#[derive(Clone)]
pub struct EventBus {
    tx: broadcast::Sender<CraftyEvent>,
...
impl EventBus {
    ...
    #[must_use]
    pub fn emit(&self, event: CraftyEvent) -> usize {
...
pub struct EventSubscription {
    rx: broadcast::Receiver<CraftyEvent>,
    dropped: Arc<AtomicU64>,
}
...
impl EventSubscription {
    ...
    pub async fn recv(&mut self) -> Option<CraftyEvent> {
        loop {
...
mod tests {
    ...
    async fn subscribers_receive_emitted_events() {
        ...
        assert_eq!(bus.emit(CraftyEvent::NodeJoined { node_id: 3 }), 1);
        assert_eq!(
            sub.recv().await,
            Some(CraftyEvent::NodeJoined { node_id: 3 })
        );
    }
