...
use craft_actor::craft_proto::{ActorId, ActorRegistration, ActorTypeId, NodeId};
use craft_actor::{
...
impl UserActor for Worker {
    ...
    fn encode_config(config: &Self::Config) -> Result<Vec<u8>, ConfigCodecError> {
        craft_actor::craft_proto::encode(config).map_err(|e| ConfigCodecError::Codec(e.to_string()))
    }

    fn decode_config(bytes: &[u8]) -> Result<Self::Config, ConfigCodecError> {
        craft_actor::craft_proto::decode(bytes).map_err(|e| ConfigCodecError::Codec(e.to_string()))
    }
}
