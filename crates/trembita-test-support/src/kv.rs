...
//!
//! Canonical implementation: [`trembita_core::kv`]. This module re-exports it
...
impl StateMachine for TrackedKv {
    ...
    fn snapshot(&self) -> Result<Vec<u8>, KvError> {
        trembita_proto::encode(self).map_err(|_| KvError)
    }

    fn restore(&mut self, snapshot: &[u8]) -> Result<(), KvError> {
        *self = trembita_proto::decode(snapshot).map_err(|_| KvError)?;
        Ok(())
    }
}
