...
use craft_actor::craft_proto::{self, ActorId, ActorRegistration, ActorTypeId, NodeId};
use craft_actor::{
    ActorDirectory, ActorRegistry, ClusterControl, ClusterScaleError, ConfigCodecError,
    RemoteSpawnError, ScaleError, UserActor, plan_scale,
};
...
async fn spawn_remote_for_an_unregistered_type_is_rejected() {
    ...
    let err = n1
        ...
        .unwrap_err();
    match err {
        RemoteSpawnError::Remote { node, reason } => {
