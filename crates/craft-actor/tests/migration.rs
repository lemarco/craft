...
use craft_actor::craft_proto::{self, ActorId, NodeId};
use craft_actor::{
...
async fn drain_times_out_and_rejects_new_messages() {
    ...
    assert_eq!(
        actor.send(()),
        Err(craft_actor::SendError::Draining),
...
impl UserActor for Counter {
    ...
    fn migration_snapshot(&self) -> Result<Vec<u8>, MigrationError> {
        craft_proto::encode(&self.count).map_err(MigrationError::new)
    }

    fn restore_migration(&mut self, snapshot: &[u8]) -> Result<(), MigrationError> {
        self.count = craft_proto::decode(snapshot).map_err(MigrationError::new)?;
        Ok(())
    }
}
