...
//!
//! Talks to a running `trembita-node` cluster (Demo state machine: each propose
//! increments a counter; query returns the current count) and checks the
//! recorded history with [`trembita_sim::History`] + [`trembita_sim::Model`].
...
use trembita_net::{CertPaths, PeerDirectory, QuicTransport, client_config, client_endpoint};
use trembita_proto::NodeId;
use trembita_sim::{History, Model};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let node_id = NodeId(
        env("TREMBITA_NODE_ID")?
            .parse()
            .map_err(|_| "TREMBITA_NODE_ID must be u64")?,
    );
    ...
    let rounds: u64 = env::var("TREMBITA_LIN_ROUNDS")
        .ok()
