//! Cross-shard two-phase commit coordinator (optional cross-shard 2PC increment).

use std::future::Future;
use std::pin::Pin;

use trembita_core::{TwoPhasePlan, TwoPhasePlanError, validate_two_phase_plan};
...
pub fn encode_two_phase_journal_record(
    ...
    trembita_proto::encode(record).map_err(|e| TwoPhaseJournalError::Codec(e.to_string()))
}
...
pub fn decode_two_phase_journal_record(
    ...
    trembita_proto::decode(bytes).map_err(|e| TwoPhaseJournalError::Codec(e.to_string()))
}
...
mod tests {
    ...
    impl Transport for TwoPhaseScript {
        fn send(
            ...
            _peer: NodeId,
            _route: Route,
            body: trembita_net::transport::Body,
        ) -> trembita_net::transport::BoxFuture<
            'static,
            Result<trembita_net::transport::Body, TransportError>,
        > {
