...
//! TREMBITA_OPS_S3_BUCKET=trembita-test \
//! cargo test -p trembita-ops --test s3_smoke -- --ignored
//! ```

use std::env;

use tempfile::tempdir;
use trembita_tools::ops::backup::{export_local, import_local, pull_object, push_object};
...
async fn s3_push_pull_roundtrip_when_configured() {
    ...
    let bucket = env::var("TREMBITA_OPS_S3_BUCKET").unwrap_or_else(|_| "trembita-test".into());
