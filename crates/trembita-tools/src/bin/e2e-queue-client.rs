...
//!
//! Phases (via `TREMBITA_E2E_QUEUE_PHASE`):
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let phase = env("TREMBITA_E2E_QUEUE_PHASE")?;
    let stream = queue_stream();

    let submit = node_from_env("TREMBITA_QUEUE_SUBMIT_NODE").unwrap_or(NodeId(1));
    let worker_peer = node_from_env("TREMBITA_QUEUE_WORKER_PEER").unwrap_or(NodeId(2));
    ...
    match phase.as_str() {
        ...
        "after_failover" => {
            let contact = node_from_env("TREMBITA_QUEUE_CONTACT_NODE").unwrap_or(submit);
            after_failover(transport, &stream, contact, worker_node, worker_instance).await?;
        }
        other => return Err(format!("unknown TREMBITA_E2E_QUEUE_PHASE {other:?}").into()),
    }
