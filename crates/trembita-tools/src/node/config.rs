//! Environment-driven configuration for the `trembita-node` reference binary.
...
pub struct NodeConfig {
    ...
    /// On shutdown, remove this node from the cluster via [`trembita::cluster::TrembitaCluster::leave`]
    ...
    /// Persistent data directory (`TREMBITA_DATA_DIR`); required when a job queue is enabled.
    pub data_dir: Option<PathBuf>,
    /// Job queue stream name (`TREMBITA_JOB_QUEUE`); enables leader queue service + redb backend.
...
pub struct DnsSpec {
    /// Hostname prefix (e.g. `trembita` → `trembita-0`, `trembita-1`, …).
    pub prefix: String,
    /// DNS zone / suffix (e.g. `trembita.internal`).
    pub service: String,
    /// Expected node count.
    pub replicas: u64,
...
/// Parse `TREMBITA_NODE_ID` (default `1` when unset).
pub fn parse_node_id(trembita_node_id: Option<&str>) -> Result<NodeId, Box<dyn Error>> {
    if let Some(raw) = trembita_node_id {
...
pub fn node_id_from_env() -> Result<NodeId, Box<dyn Error>> {
    parse_node_id(env("TREMBITA_NODE_ID").as_deref())
}
...
pub fn load_security(
    ...
    match (pem.node_cert, pem.node_key, pem.ca_cert) {
        ...
        (None, None, None) => {
            if members.len() > 1 || joining {
                return Err("multi-node clusters need shared certs: set \
                     TREMBITA_NODE_CERT/TREMBITA_NODE_KEY/TREMBITA_CA_CERT on every node \
        ...
        _ => Err(
            "set all of TREMBITA_NODE_CERT, TREMBITA_NODE_KEY, TREMBITA_CA_CERT together, \
...
pub fn load_security_from_env(
    ...
    load_security(
        ...
        joining,
        &PemEnv {
            node_cert: env("TREMBITA_NODE_CERT").as_deref(),
            node_key: env("TREMBITA_NODE_KEY").as_deref(),
            ca_cert: env("TREMBITA_CA_CERT").as_deref(),
        },
    )
}

/// Parse `TREMBITA_JOIN_SEEDS` (`id@host:port,...`) into a discovery seed set.
pub fn parse_seeds(raw: &str) -> Result<Vec<Seed>, Box<dyn Error>> {
    let mut seeds = Vec::new();
    for entry in raw.split(',').map(str::trim).filter(|e| !e.is_empty()) {
        let (id, addr) = entry.split_once('@').ok_or_else(|| {
            format!("bad TREMBITA_JOIN_SEEDS entry {entry:?} (want id@host:port)")
        })?;
        let id: u64 = id
            .parse()
...
/// Parse `TREMBITA_DISCOVERY=dns:<prefix>:<service>:<replicas>:<port>`.
pub fn parse_discovery(raw: &str) -> Result<DnsSpec, Box<dyn Error>> {
    ...
    match parts.as_slice() {
        ...
        _ => Err(format!(
            "bad TREMBITA_DISCOVERY {raw:?} (want dns:<prefix>:<service>:<replicas>:<port>)"
        )
        .into()),
    }
}
...
/// Parse `TREMBITA_PEERS` (`id@host:port,...`) into an address book + member list.
...
pub fn config_from_env() -> Result<NodeConfig, Box<dyn Error>> {
    ...
    let discovery = match env("TREMBITA_DISCOVERY") {
    ...
    if job_queue_stream.is_some() && data_dir.is_none() {
        return Err("TREMBITA_JOB_QUEUE requires TREMBITA_DATA_DIR for redb queue files".into());
    }
...
/// Read [`parse_drain_timeout`] from `TREMBITA_DRAIN_TIMEOUT`.
#[must_use]
pub fn drain_timeout_from_env() -> Duration {
    parse_drain_timeout(env("TREMBITA_DRAIN_TIMEOUT").as_deref())
}

/// Parse a cert-watch period (`TREMBITA_CERT_WATCH_SECS`, default 60).
...
pub fn cert_watch_period_from_env() -> Duration {
    parse_cert_watch_period(env("TREMBITA_CERT_WATCH_SECS").as_deref())
}
...
mod tests {
    ...
    const TREMBITA_ENV_KEYS: &[&str] = &[
        ...
        "TREMBITA_DISCOVERY",
        "TREMBITA_ALLOW_JOIN",
        "TREMBITA_ALLOW_LEAVE",
        "TREMBITA_GRACEFUL_LEAVE",
        "TREMBITA_NODE_CERT",
        "TREMBITA_NODE_KEY",
        "TREMBITA_CA_CERT",
        "TREMBITA_CERT_WATCH_SECS",
        "TREMBITA_DRAIN_TIMEOUT",
        "TREMBITA_DATA_DIR",
        "TREMBITA_JOB_QUEUE",
        "TREMBITA_JOB_QUEUE_LEASE_SECS",
    ];

    fn without_trembita_env(f: impl Fn() + std::panic::UnwindSafe + std::panic::RefUnwindSafe) {
        let clears = TREMBITA_ENV_KEYS
            .iter()
    ...
    fn explicit_pem_paths_load_for_single_node() {
        ...
        with_trembita_env(
            &[
                ("TREMBITA_NODE_CERT", Some(node_cert.to_str().unwrap())),
                ("TREMBITA_NODE_KEY", Some(node_key.to_str().unwrap())),
                ("TREMBITA_CA_CERT", Some(ca_cert.to_str().unwrap())),
            ],
            || {
    ...
    fn node_id_parses_from_env_or_defaults() {
        without_trembita_env(|| assert_eq!(node_id_from_env().unwrap(), NodeId(1)));
        with_trembita_env(&[("TREMBITA_NODE_ID", Some("7"))], || {
    ...
    fn cert_watch_period_defaults_and_parses() {
        without_trembita_env(|| {
            assert_eq!(cert_watch_period_from_env(), Duration::from_secs(60));
        });
        with_trembita_env(&[("TREMBITA_CERT_WATCH_SECS", Some("15"))], || {
    ...
    fn parse_discovery_parses_dns_spec() {
        let spec = parse_discovery("dns:trembita:trembita-headless:3:7443").expect("discovery");
        assert_eq!(spec.prefix, "trembita");
        assert_eq!(spec.service, "trembita-headless");
