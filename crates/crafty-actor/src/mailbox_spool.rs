...
use crafty_proto::{ActorEnvelope, decode, encode};
...
fn backend(e: impl std::fmt::Display) -> MailboxSpoolError {
    MailboxSpoolError::Backend(e.to_string())
}

fn codec(e: impl std::fmt::Display) -> MailboxSpoolError {
    MailboxSpoolError::Codec(e.to_string())
}
...
mod tests {
    use super::*;
    use crafty_proto::ActorId;

    fn sample_envelope() -> ActorEnvelope {
        ActorEnvelope {
            to: ActorId {
                node: crafty_proto::NodeId(2),
                name: "w".into(),
                instance: 0,
                generation: 1,
            },
            from: None,
            origin: Some(crafty_proto::NodeId(1)),
            req_id: 42,
            payload: b"hi".to_vec(),
