...
use crafty_net::transport::{Body, BoxFuture, RequestHandler};
use crafty_net::{Route, Transport, TransportError, decode_body, encode_body};
use crafty_proto::{ClientRequest, ClientResponse, GroupPeerEnvelope, NodeId};
use crafty_storage::{GroupRedbLayout, RaftStorage, StorageError};
    ...
    M: StateMachine + Default + 'static,
{
    ...
    let storage: Box<dyn RaftStorage> = if let Some(dir) = storage_dir {
        ...
        crafty_storage::import_migration(&mut storage, bundle)?;
        Box::new(storage)
    } else {
        crafty_storage::import_migration_boxed(bundle)?
    };
...
/// bootstrap voters are chosen by [`group_voters`](crafty_core::group_voters) over
