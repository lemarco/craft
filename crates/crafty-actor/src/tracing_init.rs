...
//! come from `RUST_LOG`, then `CRAFTY_LOG` if `RUST_LOG` is unset. The legacy
//! `CRAFTY_LOG_REBALANCE=1` knob adds `crafty::rebalance=debug` to the filter.
...
fn env_filter() -> tracing_subscriber::EnvFilter {
    ...
    if std::env::var_os("CRAFTY_LOG_REBALANCE").is_some() {
        ...
        directives.push_str("crafty::rebalance=debug");
    }
