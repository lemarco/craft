...
//!
//! `crafty-core`'s [`RaftNode`] is a pure, I/O-free state machine: it accepts
...
//!   caller (an actor task wrapping a `crafty-net` transport) to dispatch.
...
//! and routes the returned [`NetEffect`]s. The async actor + `crafty-net`
...
//! the core's [`Persist`](crafty_core::Persist) delta and writes the hard state
...
use crafty_storage::{HardState, NullStorage, RaftStorage, Snapshot, SnapshotMeta, StorageError};
...
impl<M: StateMachine> RaftDriver<M> {
    ...
    fn export_migration_from_live(&self) -> crafty_proto::GroupMigrationBundle {
        use crafty_proto::{
