...
use crafty_actor::crafty_core::RaftGroupId;
use crafty_actor::crafty_core::ShardRoutingKind;
use crafty_actor::crafty_net::{LocalNetwork, RequestHandler, Transport, send_client_request};
use crafty_actor::crafty_proto::{ClientRequest, ClientResponse, NodeId};
use crafty_actor::{RuntimeConfig, ShardedNodeService, spawn_multi_raft_node, spawn_raft_group};
...
async fn keyed_writes_route_to_independent_raft_groups() {
    ...
    let spawn = spawn_multi_raft_node(
        node_id,
        &members,
        crafty_actor::crafty_core::DEFAULT_GROUP_REPLICATION_FACTOR,
        crafty_actor::crafty_core::Config::default(),
        &runtime,
        &runtime_meta,
        shard_count,
    ...
    let cmd_a = crafty_actor::crafty_proto::encode(&KvCommand::Set {
        key: "a".into(),
        value: "group0".into(),
    })
    .unwrap();
    let cmd_b = crafty_actor::crafty_proto::encode(&KvCommand::Set {
        key: "a".into(),
        value: "group1".into(),
    })
    .unwrap();
    ...
    let set_a: KvResponse = crafty_actor::crafty_proto::decode(&bytes).unwrap();
    ...
    let qry_a = crafty_actor::crafty_proto::encode(&KvQuery::Get { key: "a".into() }).unwrap();
    ...
    let val_a: KvResponse = crafty_actor::crafty_proto::decode(&bytes).unwrap();
    assert_eq!(val_a, KvResponse::Value(Some("group0".into())));

    let qry_b = crafty_actor::crafty_proto::encode(&KvQuery::Get { key: "a".into() }).unwrap();
    ...
    let val_b: KvResponse = crafty_actor::crafty_proto::decode(&bytes).unwrap();
...
async fn stable_shard_activation_rejects_inactive_keys() {
    use crafty_actor::crafty_core::{ShardRoutingKind, StableShardRouter};
    ...
    let cmd = crafty_actor::crafty_proto::encode(&KvCommand::Set {
        key: "k".into(),
        value: "v".into(),
    })
    .unwrap();
