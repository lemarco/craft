...
use crafty_actor::{ClusterState, RaftGroupReconciler};
use crafty_core::RaftGroupId;
...
fn leader_plans_adopt_for_a_new_node() {
    ...
    let assignment = crafty_core::group_host_assignment(&catalog, &live);
    let node_id = live
        .iter()
        .copied()
...
fn learner_only_node_adopts_groups_when_lf_positive() {
    ...
    for group in &catalog {
        let voters = crafty_core::group_voters(*group, &live, 3);
        let learners = crafty_core::group_learners(*group, &live, 3, 1);
