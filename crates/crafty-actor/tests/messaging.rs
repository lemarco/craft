...
use crafty_actor::crafty_net::{LocalNetwork, RemoteError, Transport};
...
async fn ask_round_trips_a_reply_across_the_wire() {
    ...
    m2.cast("accum", crafty_proto::encode(&AddReq(7)).unwrap())
        .await
        .expect("remote cast");
    let reply = m2
        .ask("accum", crafty_proto::encode(&GetReq).unwrap())
        .await
        .expect("remote ask");
    let total: u64 = crafty_proto::decode(&reply).expect("decode reply");
...
async fn duplicate_ask_runs_the_handler_once_and_replays_the_reply() {
    ...
    let envelope = ActorEnvelope {
        ...
        req_id: 42,
        payload: crafty_proto::encode(&BumpReq).unwrap(),
        reply_expected: true,
    };

    let decode = |ack: &DeliverAck| -> u64 {
        crafty_proto::decode(ack.reply.as_ref().expect("reply present")).unwrap()
    };
...
async fn reply_encode_failure_surfaces_as_a_real_error() {
    ...
    let envelope = ActorEnvelope {
        ...
        origin: Some(NodeId(2)),
        req_id: 1,
        payload: crafty_proto::encode(&BadReq).unwrap(),
        reply_expected: true,
    };
...
async fn ask_linearizable_retries_until_directory_has_a_target() {
    ...
    let payload = crafty_proto::encode(&BumpReq).unwrap();
    let ask = {
    ...
    let got: u64 = crafty_proto::decode(&reply).unwrap();
    assert_eq!(got, 1);
