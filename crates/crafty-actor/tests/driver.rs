//! Integration tests for [`crafty_actor::RaftDriver`] — the runtime glue that
...
use crafty_actor::crafty_core::{Config, RaftNode, ReadId, Role, StateMachine};
...
use crafty_actor::{DriverError, NetEffect, RaftDriver, ReadOutcome, Step};
use crafty_test_support::{KvCommand, KvQuery, KvResponse, TrackedKv};

// ---------------------------------------------------------------------------
// Reference KV state machine (see `crafty-test-support`)
// ---------------------------------------------------------------------------
...
fn propose_on_follower_is_rejected_as_not_leader() {
    ...
    let err = d
        ...
        .unwrap_err();
    assert!(matches!(err, crafty_actor::DriverError::NotLeader { .. }));
}
...
fn a_follower_persists_an_installed_snapshot() {
    use crafty_actor::crafty_proto::{InstallSnapshot, LogId, Membership, RaftRpc, Term};
    ...
    origin
        ...
        .unwrap();
    let data = crafty_actor::crafty_proto::encode(&origin).unwrap();
