//! End-to-end tests for [`crafty_actor::spawn_node`] — the async node runtime.
//!
...
use crafty_actor::crafty_core::{RaftNode, Role};
use crafty_actor::crafty_net::{
...
use crafty_actor::{ClientError, NodeHandle, NodeService, RaftDriver, RuntimeConfig, spawn_node};
...
async fn follower_forwards_writes_and_serves_reads_locally() {
    ...
    let propose = ClientRequest::Propose(
        crafty_actor::crafty_proto::encode(&KvCommand::Set {
            key: "via".into(),
            value: "follower".into(),
        })
        .unwrap(),
    ...
    match response {
        ClientResponse::Ok(bytes) => {
            let decoded: KvResponse = crafty_actor::crafty_proto::decode(&bytes).unwrap();
    ...
    let query = ClientRequest::Query(
        crafty_actor::crafty_proto::encode(&KvQuery::Get { key: "via".into() }).unwrap(),
    );
...
async fn client_wire_propose_and_query_round_trip() {
    ...
    let query = ClientRequest::Query(
        crafty_actor::crafty_proto::encode(&KvQuery::Get { key: "lang".into() }).unwrap(),
    );
...
async fn auto_compaction_runs_when_entry_threshold_reached() {
    use crafty_actor::crafty_core::CompactionPolicy;
    use crafty_test_support::{await_node_leader, eventually_async_default};
