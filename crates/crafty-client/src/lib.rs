...
//!
//! * **In-process (L1):** embed a node and use its `crafty_actor::NodeHandle`
//!   directly (`propose`/`query`) — no serialization, no network.
//! * **Remote (L2):** [`RemoteClient`] speaks `postcard` over any
//!   [`crafty_net`] transport (live QUIC/HTTP/3 with client mTLS, or the
...
//! [`StateMachine`](crafty_core::StateMachine)'s command/query/response types.
