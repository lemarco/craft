...
//! (via an in-process [`NodeHandle`](trembita_runtime::NodeHandle) for zero-copy L1 clients), the actor
//! control/messaging/directory planes, the leader-only supervisor, and the
//! telemetry [`EventBus`](trembita_dashboard::EventBus) + [`Metrics`](trembita_dashboard::Metrics). Background tasks (facts refresh,
