//! [`TrembitaClusterBuilder`] configuration setters and env merge.
...
use super::types::{BuilderOverrides, UserLeaderTaskSpec};
...
impl<M: StateMachine + Default + 'static> TrembitaClusterBuilder<M> {
    ...
    /// prefix are rejected; use [`crate::TrembitaCluster::activate_shards`] to grow capacity
    ...
    /// External store for **stateful capability workflow data** (`trembita_capstore::RedbCapStateStore`
    ...
    /// [`crate::TrembitaCluster::vps_resources`] after start.
    #[must_use]
    ...
    /// Capacity of the telemetry [`trembita_dashboard::EventBus`] ring buffer per subscriber.
    #[must_use]
