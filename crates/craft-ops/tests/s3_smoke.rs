...
//! CRAFT_OPS_S3_BUCKET=craft-test \
//! cargo test -p craft-ops --test s3_smoke -- --ignored
//! ```

use std::env;

use craft_ops::backup::{export_local, import_local, pull_object, push_object};
...
async fn s3_push_pull_roundtrip_when_configured() {
    ...
    let bucket = env::var("CRAFT_OPS_S3_BUCKET").unwrap_or_else(|_| "craft-test".into());
