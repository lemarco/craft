//! Tests for the local [`trembita_runtime::ActorRegistry`] (backlog E6): singleton
...
async fn stop_removes_the_group_and_ends_instances() {
    ...
    assert!(matches!(missing, trembita_runtime::StopError::NotFound(_)));
}
...
async fn local_actor_introspection_tracks_per_instance_mailbox_depth() {
    ...
    let salt = trembita_runtime::group_salt(pool.name());
    let key_for = |target: u32| -> String {
        for n in 0..1000 {
            let key = format!("tenant-{n}");
            let idx = trembita_runtime::pick_index(trembita_runtime::ring_hash_key(&key), 2, salt);
            if ids[idx] == target {
                return key;
            }
        }
