//! End-to-end tests for [`trembita_runtime::spawn_node`] — the async node runtime.
//!
...
use trembita_runtime::trembita_core::{RaftNode, Role};
...
async fn follower_forwards_writes_and_serves_reads_locally() {
    ...
    let propose = ClientRequest::Propose(
        trembita_runtime::trembita_proto::encode(&KvCommand::Set {
            key: "via".into(),
            value: "follower".into(),
        })
        .unwrap(),
    ...
    match response {
        ClientResponse::Ok(bytes) => {
            let decoded: KvResponse = trembita_runtime::trembita_proto::decode(&bytes).unwrap();
    ...
    let query = ClientRequest::Query(
        trembita_runtime::trembita_proto::encode(&KvQuery::Get { key: "via".into() }).unwrap(),
    );
...
async fn client_wire_propose_and_query_round_trip() {
    ...
    let query = ClientRequest::Query(
        trembita_runtime::trembita_proto::encode(&KvQuery::Get { key: "lang".into() }).unwrap(),
    );
