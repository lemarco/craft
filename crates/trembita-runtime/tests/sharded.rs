...
use trembita_runtime::trembita_core::RaftGroupId;
use trembita_runtime::trembita_core::ShardRoutingKind;
...
async fn keyed_writes_route_to_independent_raft_groups() {
    ...
    let spawn = spawn_multi_raft_node(
        node_id,
        &members,
        trembita_runtime::trembita_core::DEFAULT_GROUP_REPLICATION_FACTOR,
        trembita_runtime::trembita_core::Config::default(),
        &runtime,
        &runtime_meta,
        shard_count,
    ...
    let cmd_a = trembita_runtime::trembita_proto::encode(&KvCommand::Set {
        key: "a".into(),
        value: "group0".into(),
    })
    .unwrap();
    let cmd_b = trembita_runtime::trembita_proto::encode(&KvCommand::Set {
        key: "a".into(),
        value: "group1".into(),
    })
    .unwrap();
    ...
    let set_a: KvResponse = trembita_runtime::trembita_proto::decode(&bytes).unwrap();
    ...
    let qry_a =
        trembita_runtime::trembita_proto::encode(&KvQuery::Get { key: "a".into() }).unwrap();
    ...
    let val_a: KvResponse = trembita_runtime::trembita_proto::decode(&bytes).unwrap();
    ...
    let val_b: KvResponse = trembita_runtime::trembita_proto::decode(&bytes).unwrap();
...
async fn stable_shard_activation_rejects_inactive_keys() {
    use trembita_runtime::trembita_core::{ShardRoutingKind, StableShardRouter};
    ...
    let cmd = trembita_runtime::trembita_proto::encode(&KvCommand::Set {
        key: "k".into(),
        value: "v".into(),
    })
    .unwrap();
