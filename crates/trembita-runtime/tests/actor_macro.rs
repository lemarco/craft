//! Tests for the `#[trembita_runtime::actor]` attribute (backlog D2): it
...
use trembita_runtime::{MessageDecodeError, UserActor, actor};
...
fn generated_message_decode_round_trips() {
    let bytes = trembita_runtime::trembita_proto::encode(&Msg::Add(5)).unwrap();
