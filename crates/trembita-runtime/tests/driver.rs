//! Integration tests for [`trembita_runtime::RaftDriver`] — the runtime glue that
...
use trembita_runtime::{DriverError, NetEffect, RaftDriver, ReadOutcome, Step};
...
fn propose_on_follower_is_rejected_as_not_leader() {
    ...
    assert!(matches!(
        err,
        trembita_runtime::DriverError::NotLeader { .. }
    ));
}
...
fn a_follower_persists_an_installed_snapshot() {
    use trembita_runtime::trembita_proto::{InstallSnapshot, LogId, Membership, RaftRpc, Term};
    ...
    origin
        ...
        .unwrap();
    let data = trembita_runtime::trembita_proto::encode(&origin).unwrap();
