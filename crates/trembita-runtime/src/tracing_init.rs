...
//! come from `RUST_LOG`, then `TREMBITA_LOG` if `RUST_LOG` is unset. The legacy
//! `TREMBITA_LOG_REBALANCE=1` knob adds `trembita::rebalance=debug` to the filter.
...
fn env_filter() -> tracing_subscriber::EnvFilter {
    ...
    if std::env::var_os("TREMBITA_LOG_REBALANCE").is_some() {
        ...
        directives.push_str("trembita::rebalance=debug");
    }
