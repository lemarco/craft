...
//!
//! `trembita-core`'s [`RaftNode`] is a pure, I/O-free state machine: it accepts
...
//!   caller (an actor task wrapping a `trembita-net` transport) to dispatch.
...
//! and routes the returned [`NetEffect`]s. The async actor + `trembita-net`
...
//! the core's [`Persist`](trembita_core::Persist) delta and writes the hard state
...
use trembita_core::Command as _;
use trembita_core::{
...
use trembita_storage::{HardState, NullStorage, RaftStorage, Snapshot, SnapshotMeta, StorageError};
...
impl<M: StateMachine> RaftDriver<M> {
    ...
    pub fn export_migration(
        &mut self,
    ) -> Result<trembita_proto::GroupMigrationBundle, DriverError> {
        self.persist()?;
        let bundle = trembita_storage::export_migration(self.storage.as_ref())
    ...
    fn export_migration_from_live(&self) -> trembita_proto::GroupMigrationBundle {
        use trembita_proto::{
