    ...
    M: StateMachine + Default + 'static,
{
    ...
    let storage: Box<dyn RaftStorage> = if let Some(dir) = storage_dir {
        ...
        trembita_storage::import_migration(&mut storage, bundle)?;
        Box::new(storage)
    } else {
        trembita_storage::import_migration_boxed(bundle)?
    };
...
/// bootstrap voters are chosen by [`group_voters`](trembita_core::group_voters) over
...
where
    M: StateMachine + 'static,
{
    use trembita_core::group_voters;
