//! `trembita-macros` — derive/attribute macros for the trembita framework (backlog
//! Track D).
//!
//! Public entry points are the [`macro@actor`] and [`macro@consumer`] attributes.
//! [`actor`] fills in wire codecs on a `UserActor` impl;
//! [`consumer`] generates a `JobConsumer` adapter for queue workers (re-exported by the `trembita` crate).
//!
...
///
/// ```ignore
/// #[trembita::consumer_json("emails", EmailJob)]
...
/// bodies call `trembita_proto::encode`/`decode`, the associated `Config`/`Message`
