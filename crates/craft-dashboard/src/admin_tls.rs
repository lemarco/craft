...
//!
//! Distinct from the mTLS craft wire: admin TLS terminates HTTPS for probes and
...
pub struct AdminTlsPaths {
    /// Server certificate chain (`CRAFT_ADMIN_TLS_CERT`).
    pub cert: PathBuf,
    /// Private key (`CRAFT_ADMIN_TLS_KEY`).
    pub key: PathBuf,
}
