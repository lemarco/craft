...
async fn spawn(ready: bool) -> (SocketAddr, Metrics, EventBus) {
    ...
    metrics.incr("craft_client_requests_total", "Client requests.", &[], 7.0);
...
async fn metrics_render_prometheus_text() {
    ...
    assert_eq!(status, 200);
    assert!(body.contains("# TYPE craft_client_requests_total counter"));
    assert!(body.contains("craft_client_requests_total 7"));
}
...
async fn dashboard_serves_html() {
    ...
    assert_eq!(status, 200);
    assert!(body.contains("<title>craft · dashboard</title>"));
}
