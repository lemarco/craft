...
async fn pem_hot_reload_reissues_leaf_without_restart() {
    ...
    let cluster1 = CraftyCluster::builder(NodeId(1), Counter::default())
    ...
    let cluster2 = CraftyCluster::builder(NodeId(2), Counter::default())
        .members([NodeId(1), NodeId(2)])
        .raft_config(raft)
        .tick_period(Duration::from_millis(10))
...
impl crafty::core::StateMachine for Counter {
    type Command = u64;
    type Query = ();
    type Response = u64;
    type Error = std::convert::Infallible;

    fn apply(&mut self, _: crafty::proto::LogIndex, cmd: &u64) -> Result<u64, Self::Error> {
        self.0 += *cmd;
        Ok(self.0)
    }
