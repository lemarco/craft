...
use crafty::{
    ConsumerOpts, CraftyApp, CraftyConfigure, GatewayIdentity, GatewayOpts, GatewayRequest,
    IdentityError, QueueOpts, consumer,
};
...
async fn gateway_jobs_batch_and_job_status_metadata() {
    ...
    let job_id = app
        .enqueue_opts(
            "gateway-jobs",
            b"lookup",
            crafty::cluster::EnqueueOptions::dedup_key("lookup-key"),
        )
        .await
        .expect("enqueue");
