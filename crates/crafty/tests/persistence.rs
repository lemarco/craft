//! Facade persistence: `CraftyClusterBuilder::data_dir` survives stop → restart.
...
where
    M: crafty::core::StateMachine + Send + Sync + Default + 'static,
{
    CraftyCluster::builder(id, machine)
        .members(members)
...
async fn compacted_snapshot_survives_facade_restart() {
    ...
    {
        let cluster = CraftyCluster::builder(node_id, TrackedKv::default())
            .members([node_id])
...
async fn three_node_majority_survives_one_member_restart() {
    ...
    let mut clusters: Vec<Arc<CraftyCluster<KvMachine>>> = Vec::new();
    for &id in &members {
