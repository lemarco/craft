//! [`RemoteClient`] keyed propose/query against a multi-Raft [`CraftyCluster`].
...
use crafty_test_support::{
    KvCommand, KvMachine, KvQuery, KvResponse, TICK_PERIOD, await_crafty_leader,
    fast_raft_config_with_seed, find_keys_for_two_groups, wait_for_each_group_cluster_leader,
};

async fn spawn_multi_raft_cluster() -> (LocalNetwork, Vec<Arc<CraftyCluster<KvMachine>>>) {
