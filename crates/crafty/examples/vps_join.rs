...
//!
//! Run with: `cargo run -p crafty --example vps_join --features dev-certs`
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, &id) in ids.iter().enumerate() {
        let cluster = CraftyCluster::builder(id, Counter::default())
            .members(ids)
    ...
    let joiner = CraftyCluster::builder(joiner_id, Counter::default())
...
async fn first_leader(clusters: &[CraftyCluster<Counter>]) -> Option<usize> {
...
async fn await_leader(clusters: &[CraftyCluster<Counter>]) -> usize {
    for _ in 0..1000 {
