...
//!
//! Run: `cargo run -p crafty --example job_queue_cluster`
...
use crafty::{
    AutoscalePolicy, CraftyCluster, EnqueueOptions, MembershipAutoscalePolicy, NodeId, WorkerId,
    run_queue_consumer,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base = std::env::temp_dir().join("crafty-job-queue-cluster-example");
