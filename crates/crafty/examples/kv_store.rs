//! A minimal replicated key-value store on a **single-node** crafty cluster.
//!
...
//! starting each on its own process with [`CraftyCluster::builder`]`.start_quic`.
//!
//! Run with: `cargo run -p crafty --example kv_store`
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(got, Resp::Value(Some(ref v)) if v == "hello, crafty"));
