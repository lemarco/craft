...
//!
//! Run: `cargo run -p crafty --example job_queue_worker`
...
use crafty::{CraftyCluster, NodeId, WorkerId, run_queue_consumer};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base = std::env::temp_dir().join("crafty-job-queue-worker-example");
