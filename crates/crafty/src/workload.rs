//! Per-node workload runtime wired by [`crate::cluster::CraftyClusterBuilder::workload`].

use std::sync::Arc;

use crafty_actor::{ComputeTokenPool, ConsumerTune};
...
impl WorkloadRuntime {
    ...
    pub(crate) fn queue_consumer_workload(&self) -> crafty_actor::QueueConsumerWorkload {
        crafty_actor::QueueConsumerWorkload {
            tokens: self.pool(),
            tune: self.tune(),
        }
    }
