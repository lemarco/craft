...
use crafty_core::{Config, StateMachine};
use crafty_core::{RaftGroupId, group_voters};
use crafty_dashboard::CraftyEvent;
use crafty_net::{Transport, send_group_migrate};
...
impl<M: StateMachine + Default + 'static> MultiRaftState<M> {
    ...
    pub async fn rebalance(
        ...
        crafty_actor::rebalance_log::line(format!(
        ...
        for group in &report.plan.adopt {
            ...
            crafty_actor::rebalance_log::line(format!("node={} adopt group={id}", self.node_id.0));
    ...
    pub fn emit_rebalance(events: &crafty_dashboard::EventBus, report: &GroupRebalanceReport) {
        if report.plan.adopt.is_empty() && report.plan.retire.is_empty() {
            return;
        }
        let _ = events.emit(CraftyEvent::RaftGroupsRebalanced {
...
pub(crate) trait GroupMigratePort: Send + Sync {
    ...
    fn handle_group_migrate(
        &self,
        request: GroupMigrateRequest,
    ) -> crafty_net::transport::BoxFuture<'static, GroupMigrateReply>;
}
