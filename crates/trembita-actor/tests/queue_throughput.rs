//! [`QueueService`] batch + prefetch over [`LocalNetwork`](trembita_net::LocalNetwork).

use std::sync::Arc;
use std::time::Duration;

use trembita_actor::{ClusterState, DEFAULT_QUEUE_PREFETCH, JobQueue, QueueService, RedbJobQueue};
...
async fn queue_service_batch_prefetch_priority_and_ack_eviction() {
    ...
    let worker = trembita_actor::WorkerId {
        node: NodeId(1),
        instance: 0,
    };
