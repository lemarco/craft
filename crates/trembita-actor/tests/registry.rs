//! Tests for the local [`trembita_actor::ActorRegistry`] (backlog E6): singleton
...
async fn stop_removes_the_group_and_ends_instances() {
    ...
    assert!(matches!(missing, trembita_actor::StopError::NotFound(_)));
}
...
async fn local_actor_introspection_tracks_per_instance_mailbox_depth() {
    ...
    let salt = trembita_actor::group_salt(pool.name());
    let key_for = |target: u32| -> String {
        for n in 0..1000 {
            let key = format!("tenant-{n}");
            let idx = trembita_actor::pick_index(trembita_actor::ring_hash_key(&key), 2, salt);
            if ids[idx] == target {
                return key;
            }
        }
