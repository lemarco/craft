...
use trembita_actor::trembita_core::RaftGroupId;
use trembita_actor::trembita_core::ShardRoutingKind;
use trembita_actor::trembita_net::{LocalNetwork, RequestHandler, Transport, send_client_request};
use trembita_actor::trembita_proto::{ClientRequest, ClientResponse, NodeId};
use trembita_actor::{RuntimeConfig, ShardedNodeService, spawn_multi_raft_node, spawn_raft_group};
...
async fn keyed_writes_route_to_independent_raft_groups() {
    ...
    let spawn = spawn_multi_raft_node(
        node_id,
        &members,
        trembita_actor::trembita_core::DEFAULT_GROUP_REPLICATION_FACTOR,
        trembita_actor::trembita_core::Config::default(),
        &runtime,
        &runtime_meta,
        shard_count,
    ...
    let cmd_a = trembita_actor::trembita_proto::encode(&KvCommand::Set {
        key: "a".into(),
        value: "group0".into(),
    })
    .unwrap();
    let cmd_b = trembita_actor::trembita_proto::encode(&KvCommand::Set {
        key: "a".into(),
        value: "group1".into(),
    })
    .unwrap();
    ...
    let set_a: KvResponse = trembita_actor::trembita_proto::decode(&bytes).unwrap();
    ...
    let qry_a = trembita_actor::trembita_proto::encode(&KvQuery::Get { key: "a".into() }).unwrap();
    ...
    let val_a: KvResponse = trembita_actor::trembita_proto::decode(&bytes).unwrap();
    assert_eq!(val_a, KvResponse::Value(Some("group0".into())));

    let qry_b = trembita_actor::trembita_proto::encode(&KvQuery::Get { key: "a".into() }).unwrap();
    ...
    let val_b: KvResponse = trembita_actor::trembita_proto::decode(&bytes).unwrap();
...
async fn stable_shard_activation_rejects_inactive_keys() {
    use trembita_actor::trembita_core::{ShardRoutingKind, StableShardRouter};
    ...
    let cmd = trembita_actor::trembita_proto::encode(&KvCommand::Set {
        key: "k".into(),
        value: "v".into(),
    })
    .unwrap();
