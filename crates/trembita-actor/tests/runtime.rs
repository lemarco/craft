//! End-to-end tests for [`trembita_actor::spawn_node`] — the async node runtime.
//!
...
use trembita_actor::trembita_core::{RaftNode, Role};
use trembita_actor::trembita_net::{
...
use trembita_actor::trembita_storage::{
    HardState, HardStateStore, LogStore, MemoryStorage, Snapshot, SnapshotStore, StorageError,
};
use trembita_actor::{ClientError, NodeHandle, NodeService, RaftDriver, RuntimeConfig, spawn_node};
...
impl Cluster {
    ...
    fn spawn_one(&mut self, id: NodeId, members: impl IntoIterator<Item = NodeId>) {
        ...
        self.spawn_one_with_config(id, members, &config);
    }
...
async fn follower_forwards_writes_and_serves_reads_locally() {
    ...
    let propose = ClientRequest::Propose(
        trembita_actor::trembita_proto::encode(&KvCommand::Set {
            key: "via".into(),
            value: "follower".into(),
        })
        .unwrap(),
    ...
    match response {
        ClientResponse::Ok(bytes) => {
            let decoded: KvResponse = trembita_actor::trembita_proto::decode(&bytes).unwrap();
    ...
    let query = ClientRequest::Query(
        trembita_actor::trembita_proto::encode(&KvQuery::Get { key: "via".into() }).unwrap(),
    );
...
async fn client_wire_propose_and_query_round_trip() {
    ...
    let query = ClientRequest::Query(
        trembita_actor::trembita_proto::encode(&KvQuery::Get { key: "lang".into() }).unwrap(),
    );
...
async fn a_new_node_joins_as_voter_when_enabled() {
    ...
    for &id in &ids {
        cluster.spawn_one_with_config(id, ids, &config);
        cluster.ids.push(id);
    }
    let leader = cluster.wait_for_leader().await;
    ...
    let joiner = NodeId(4);
    let full = [NodeId(1), NodeId(2), NodeId(3), NodeId(4)];
    let joiner_config = RuntimeConfig {
        tick_period: TICK_PERIOD,
        allow_join: true,
        allow_leave: true,
        allow_voter_join: true,
    ...
    cluster.spawn_one_with_config(joiner, full.iter().copied(), &joiner_config);
    cluster.ids.push(joiner);
...
async fn unreachable_voter_is_replaced_by_caught_up_learner() {
    ...
    let joiner = NodeId(4);
    let full = [NodeId(1), NodeId(2), NodeId(3), NodeId(4)];
    cluster.spawn_one_with_config(joiner, full.iter().copied(), &config);
    cluster.ids.push(joiner);
    ...
    let replaced = tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if let Some(status) = cluster.handles[&leader].status().await
                && status.voters.contains(&joiner)
                && !status.voters.contains(&victim)
            {
                return status;
            }
            advance(TICK_PERIOD).await;
...
async fn auto_compaction_runs_when_entry_threshold_reached() {
    use trembita_actor::trembita_core::CompactionPolicy;
