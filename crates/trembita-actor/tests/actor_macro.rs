//! Tests for the `#[trembita_actor::actor]` attribute (backlog D2): it
...
use trembita_actor::{MessageDecodeError, UserActor, actor};
...
fn generated_message_decode_round_trips() {
    let bytes = trembita_actor::trembita_proto::encode(&Msg::Add(5)).unwrap();
