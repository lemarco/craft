//! Integration tests for [`trembita_actor::RaftDriver`] — the runtime glue that
...
use trembita_actor::trembita_core::{Config, RaftNode, ReadId, Role, StateMachine};
...
use trembita_actor::{DriverError, NetEffect, RaftDriver, ReadOutcome, Step};
use trembita_test_support::{KvCommand, KvQuery, KvResponse, TrackedKv};

// ---------------------------------------------------------------------------
// Reference KV state machine (see `trembita-test-support`)
// ---------------------------------------------------------------------------
...
fn propose_on_follower_is_rejected_as_not_leader() {
    ...
    let err = d
        ...
        .unwrap_err();
    assert!(matches!(err, trembita_actor::DriverError::NotLeader { .. }));
}
...
fn a_follower_persists_an_installed_snapshot() {
    use trembita_actor::trembita_proto::{InstallSnapshot, LogId, Membership, RaftRpc, Term};
    ...
    origin
        ...
        .unwrap();
    let data = trembita_actor::trembita_proto::encode(&origin).unwrap();
