...
fn group_node_weight(group: RaftGroupId, node: trembita_proto::NodeId) -> u64 {
...
pub(crate) fn place_group(
    group: RaftGroupId,
    nodes: &[trembita_proto::NodeId],
) -> Option<trembita_proto::NodeId> {
    nodes
        .iter()
        .copied()
...
pub fn group_host_assignment(
    groups: &[RaftGroupId],
    nodes: &[trembita_proto::NodeId],
) -> BTreeMap<RaftGroupId, trembita_proto::NodeId> {
...
pub fn group_voters(
    group: RaftGroupId,
    live_nodes: &[trembita_proto::NodeId],
    replication_factor: u32,
) -> Vec<trembita_proto::NodeId> {
...
pub struct GroupReplicationTarget {
    ...
    pub voters: Vec<trembita_proto::NodeId>,
    /// Non-voting learners (catch-up replicas).
    pub learners: Vec<trembita_proto::NodeId>,
}
...
pub fn group_membership_assignment(
    ...
    live_nodes: &[trembita_proto::NodeId],
    replication_factor: u32,
) -> BTreeMap<RaftGroupId, Vec<trembita_proto::NodeId>> {
    groups
        .iter()
...
pub struct GroupMembershipChange {
    ...
    pub add: Vec<trembita_proto::NodeId>,
    /// Voters to remove via joint consensus.
    pub remove: Vec<trembita_proto::NodeId>,
}

/// Diff `current_voters` against `desired_voters` (sorted inputs not required).
#[must_use]
pub fn plan_group_membership_change(
    current_voters: &[trembita_proto::NodeId],
    desired_voters: &[trembita_proto::NodeId],
...
pub fn groups_joining_node_affects(
    node: trembita_proto::NodeId,
    all_groups: &[RaftGroupId],
    live_nodes_before: &[trembita_proto::NodeId],
    live_nodes_after: &[trembita_proto::NodeId],
    replication_factor: u32,
) -> Vec<RaftGroupId> {
    debug_assert!(
...
pub fn plan_group_membership_sync(
    catalog: &[RaftGroupId],
    live_nodes: &[trembita_proto::NodeId],
    current_voters: &BTreeMap<RaftGroupId, Vec<trembita_proto::NodeId>>,
    current_learners: &BTreeMap<RaftGroupId, Vec<trembita_proto::NodeId>>,
    replication_factor: u32,
    learner_factor: u32,
