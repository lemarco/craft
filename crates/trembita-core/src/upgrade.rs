...
//! Download, binary replace, and process exit live in the `trembita` facade (`upgrade` module).
...
pub struct ArtifactManifest {
    /// Application semver after restart (`TREMBITA_APP_VERSION` / join skew).
    pub app_version: String,
    /// Download URL (`https://…` or `file://…` for local demos).
    pub url: String,
    /// SHA-256 of the artifact bytes (hex-encoded).
    pub sha256_hex: String,
    /// Optional lower bound for [`trembita_proto::PROTOCOL_VERSION`] compatibility.
    #[serde(default)]
