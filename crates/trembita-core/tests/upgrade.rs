...
use trembita_core::proto::LogIndex;
use trembita_core::{
...
fn upgrade_command_postcard_roundtrip() {
    ...
    let manifest_bytes = trembita_proto::encode(&manifest).expect("manifest encode");
    trembita_proto::decode::<ArtifactManifest>(&manifest_bytes).expect("manifest decode");
