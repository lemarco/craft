...
use trembita_proto::{
    AppendEntries, AutoscalePolicyWire, EntryPayload, LogEntry, LogId, LogIndex, LogicalTick,
    NodeId, QueueAutoscalePolicyCommand, RaftRpc, Round, Term,
};
...
fn elect_leader_term1(n: &mut RaftNode) {
    ...
    n.receive_reply(
        NodeId(2),
        trembita_proto::RaftRpcReply::RequestVote(trembita_proto::RequestVoteReply {
            term: Term(1),
            vote_granted: true,
            pre_vote: false,
        }),
