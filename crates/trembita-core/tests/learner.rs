...
fn learner_does_not_start_election_on_timeout() {
    ...
    assert!(
        n.take_outputs()
            .iter()
            .all(|o| !matches!(o, trembita_core::Output::Send(_, RaftRpc::RequestVote(_)))),
...
fn learner_refuses_to_grant_votes() {
    ...
    let reply = outs
        .iter()
        .find_map(|o| match o {
            trembita_core::Output::Reply(_, RaftRpcReply::RequestVote(r)) => Some(r),
            _ => None,
        })
        .expect("vote reply");
...
fn leader_replicates_to_committed_learner() {
    ...
    assert!(
        outs.iter().any(|o| matches!(
            o,
            trembita_core::Output::Send(NodeId(4), RaftRpc::AppendEntries(_))
        )),
