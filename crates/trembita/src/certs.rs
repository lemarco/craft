...
pub struct ReloadOpts {
    ...
    /// roll followers first ([certificates](https://gitlab.com/lemarco/trembita/-/blob/main/docs/decisions/certificates.md)).
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match self.reload_now(ReloadOpts::default()).await {
                    ...
                    Err(e) => eprintln!("trembita cert reload failed: {e}"),
                }
            }
        })
    }
            ...
            Some(tokio::spawn(async move {
                ...
                while stream.recv().await.is_some() {
                    if let Err(e) = self.reload_now(ReloadOpts::default()).await {
                        eprintln!("trembita cert reload (SIGHUP): {e}");
                    }
                }
            }))
        }
...
/// Per-node PEM paths under a shared cert directory (`TREMBITA_CERT_DIR`).
#[must_use]
