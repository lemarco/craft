...
impl<M: StateMachine + Default + 'static> MultiRaftState<M> {
    ...
    pub fn emit_rebalance(events: &trembita_dashboard::EventBus, report: &GroupRebalanceReport) {
        if report.plan.adopt.is_empty() && report.plan.retire.is_empty() {
            return;
        }
        let _ = events.emit(TrembitaEvent::RaftGroupsRebalanced {
...
pub(crate) trait GroupMigratePort: Send + Sync {
    ...
    fn handle_group_migrate(
        &self,
        request: GroupMigrateRequest,
    ) -> trembita_net::transport::BoxFuture<'static, GroupMigrateReply>;
}
