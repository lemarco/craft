//! Queue consumer helpers ([`JobConsumer`], [`ConsumerOpts`], [`TrembitaApp::spawn_consumer`]).
...
pub(crate) type ConsumerSpawnFn = Box<
    dyn FnOnce(Arc<TrembitaApp>, tokio::sync::watch::Receiver<bool>) -> tokio::task::JoinHandle<()>
        + Send,
>;
...
/// Options for [`TrembitaApp::spawn_consumer`].
#[derive(Clone)]
pub struct ConsumerOpts {
    ...
    on_app: Option<Arc<dyn Fn(Arc<TrembitaApp>) + Send + Sync>>,
}
...
impl ConsumerOpts {
    ...
    /// Run `hook` once when this consumer task starts (e.g. register `Arc<TrembitaApp>` for handlers).
    #[must_use]
    pub fn on_app<F>(mut self, hook: F) -> Self
    where
        F: Fn(Arc<TrembitaApp>) + Send + Sync + 'static,
    {
...
/// [`TrembitaApp::spawn_consumer`].
///
/// When [`Self::SUBSCRIPTION`] is set, the handler is a topic subscriber on
/// [`Self::STREAM`] and [`TrembitaApp::spawn_consumer`] runs [`run_topic_subscriber`]
