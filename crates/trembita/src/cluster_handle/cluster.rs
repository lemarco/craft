...
use trembita_dashboard::{EventBus, Metrics, Observer, TraceOpts, TrembitaEvent};
...
use trembita_proto::{
    CatalogAddRequest, CatalogAddResponse, LeaveRejection, LeaveRequest, LeaveResponse, Membership,
    NodeId, PROTOCOL_VERSION, ScaleRequest,
};
use trembita_runtime::{
    ActorDirectory, ActorRegistry, ClusterControl, ClusterMessaging, ClusterSupervisor,
    DirectorySync, NOT_LEADER_REASON, NodeHandle, NodeStatus, ResourceProfile, UserActor,
    VpsResources,
};
...
use super::errors::{
    AddRaftGroupsError, CATALOG_ADD_RETRY, CATALOG_ADD_TIMEOUT, CatalogAddLocalError, LEAVE_RETRY,
    LEAVE_TIMEOUT, LeaveError, SCALE_FORWARD_RETRY, SCALE_FORWARD_TIMEOUT, ScaleClusterError,
};
use super::facts::ClusterFacts;
...
pub struct TrembitaCluster<M: StateMachine> {
...
impl<M: StateMachine> TrembitaCluster<M> {
    ...
    pub fn keyed_client(&self) -> Arc<trembita_client::RemoteClient> {
        Arc::new(trembita_client::RemoteClient::new(
    ...
    #[must_use]
    pub fn saga_journal(&self) -> Arc<dyn trembita_client::SagaJournal> {
    ...
    /// In single-group mode when group 0 is not hosted on this node.
    #[must_use]
    pub fn two_phase_journal(&self) -> Arc<dyn trembita_client::TwoPhaseJournal> {
        let raft_journal = if let Some(meta) = &self.meta_handle {
    ...
    #[must_use]
    pub fn shard_routing(&self) -> trembita_core::ShardRoutingKind {
        if let Some(mr) = &self.multi_raft {
    ...
    ///
    /// # Errors
    /// Returns [`trembita_core::ShardExpansionError::NotMultiRaft`] on single-Raft
    /// clusters, [`trembita_core::ShardExpansionError::StableRoutingActive`] when stable
    /// routing is configured, or planner errors when `new_count` is invalid.
    pub fn expand_shard_count(
        &self,
        new_count: u32,
    ) -> Result<trembita_core::ShardCountExpansionPlan, trembita_core::ShardExpansionError> {
        let mr = self
            .multi_raft
            .as_ref()
            .ok_or(trembita_core::ShardExpansionError::NotMultiRaft)?;
    ...
    ///
    /// # Errors
    /// Returns [`trembita_core::StableShardActivationError::NotMultiRaft`] on single-Raft
    /// clusters, [`trembita_core::StableShardActivationError::ModulusRoutingActive`] when
    /// modulus routing is configured, or planner errors when `new_active` is invalid.
    pub fn activate_shards(
        &self,
        new_active: u32,
    ) -> Result<trembita_core::StableShardActivationPlan, trembita_core::StableShardActivationError>
    {
        let mr = self
            .multi_raft
            .as_ref()
            .ok_or(trembita_core::StableShardActivationError::NotMultiRaft)?;
    ...
    ///
    /// # Errors
    /// Returns [`trembita_core::ShardRoutingSwitchError::NotMultiRaft`] on single-Raft
    /// clusters or [`trembita_core::ShardRoutingSwitchError::AlreadyStable`] when stable
    /// routing is already active.
    pub fn switch_to_stable_shards(
        &self,
    ) -> Result<trembita_core::ShardRoutingSwitchPlan, trembita_core::ShardRoutingSwitchError> {
        let mr = self
            .multi_raft
            .as_ref()
            .ok_or(trembita_core::ShardRoutingSwitchError::NotMultiRaft)?;
    ...
    pub fn record_saga_event(&self, event: trembita_client::SagaEvent) {
        crate::saga::record_saga_event(&self.metrics, self.node_id.0, event);
    }

    /// Metrics hook for [`trembita_client::RunSagaOpts::on_event`].
    #[must_use]
    pub fn saga_metrics_callback(&self) -> Arc<dyn Fn(trembita_client::SagaEvent) + Send + Sync> {
    ...
    pub fn record_two_phase_event(&self, event: trembita_client::TwoPhaseEvent) {
        crate::two_phase::record_two_phase_event(&self.metrics, self.node_id.0, event);
    }

    /// Metrics hook for [`trembita_client::RunTwoPhaseOpts::on_event`].
    #[must_use]
    pub fn two_phase_metrics_callback(
        &self,
    ) -> Arc<dyn Fn(trembita_client::TwoPhaseEvent) + Send + Sync> {
        crate::two_phase::two_phase_metrics_callback(self.metrics.clone(), self.node_id.0)
    }

    fn group_for_key(&self, key: &[u8]) -> Option<u32> {
        use trembita_core::{RaftGroupId, StableShardRouter, place_shard};
    ...
    ///
    /// # Errors
    /// Same as [`trembita_client::propose_cross_shard_2pc`].
    pub async fn run_keyed_2pc<C: trembita_client::TwoPhaseClient>(
        &self,
        client: &C,
        ...
        trembita_client::propose_cross_shard_2pc_with_opts(
            client,
            plan,
            |key| self.group_for_key(key),
            trembita_client::RunTwoPhaseOpts {
    ...
    ///
    /// # Errors
    /// Same as [`trembita_client::resume_cross_shard_2pc`].
    pub async fn resume_cross_shard_2pc<C: trembita_client::TwoPhaseClient>(
        &self,
        client: &C,
        plan: &trembita_core::TwoPhasePlan,
    ) -> Result<Vec<Vec<u8>>, trembita_client::TwoPhaseError> {
        let journal = self.two_phase_journal();
        let on_event = self.two_phase_metrics_callback();
        trembita_client::resume_cross_shard_2pc(
            client,
            plan,
            |key| self.group_for_key(key),
            trembita_client::ResumeTwoPhaseOpts {
                journal: Some(journal.as_ref()),
                probe: true,
                on_event: Some(on_event.as_ref()),
            },
    ...
    ///
    /// # Errors
    /// Same as [`trembita_client::run_saga`].
    pub async fn run_keyed_saga<C: trembita_client::KeyedClient>(
        &self,
        client: &C,
        plan: &trembita_client::SagaPlan,
        journal: &dyn trembita_client::SagaJournal,
    ) -> Result<trembita_client::SagaOutcome, trembita_client::SagaError> {
        let on_event = self.saga_metrics_callback();
        trembita_client::run_saga(
            client,
            plan,
            trembita_client::RunSagaOpts {
    ...
    ///
    /// # Errors
    /// Same as [`trembita_client::resume_saga`].
    pub async fn resume_keyed_saga<C: trembita_client::KeyedClient>(
        &self,
        client: &C,
        plan: &trembita_client::SagaPlan,
        journal: &dyn trembita_client::SagaJournal,
        ...
        let on_event = self.saga_metrics_callback();
        trembita_client::resume_saga(
            client,
            plan,
            trembita_client::ResumeSagaOpts {
                journal,
                catalog_version_live: Some(Arc::clone(&self.catalog_version)),
                on_event: Some(on_event.as_ref()),
            },
    ...
    /// [`TrembitaEvent::MessageHandled`] (with handling latency) onto
    ...
    pub async fn request_leave(
        ...
        contact: NodeId,
    ) -> Result<LeaveResponse, trembita_net::TransportError> {
        send_leave_request(
            transport,
            contact,
            &LeaveRequest {
    ...
    pub async fn add_raft_groups(&self, count: u32) -> Result<Vec<u32>, AddRaftGroupsError> {
        ...
        loop {
            ...
            if matches!(status.role, trembita_core::Role::Leader) {
                return self
...
impl<M: StateMachine> Drop for TrembitaCluster<M> {
    fn drop(&mut self) {
        self.shutdown();
    }
}
