...
//!
//! `trembita-net` attaches exactly one handler per node, and each sub-handler only
...
/// production; a no-op over the in-memory [`LocalNetwork`](trembita_net::LocalNetwork),
...
/// [`LocalNetwork`](trembita_net::LocalNetwork)): there is nothing to learn or
    ...
    fn handle(&self, route: Route, body: Body) -> BoxFuture<'static, Result<Body, TransportError>> {
        match route {
            ...
            Route::ClusterGroupMigrate => {
                ...
                Box::pin(async move {
                    let request: trembita_proto::GroupMigrateRequest = decode_body(&body)?;
