...
/// a background task. Exposed only so [`TrembitaCluster::supervisor`] has a nameable
...
/// for an actor via [`TrembitaCluster::trace`] — each handled message emits a
...
/// How long [`scale_cluster`](TrembitaCluster::scale_cluster) keeps re-resolving
...
/// Total budget for [`TrembitaCluster::leave`] peer retries.
...
/// Why [`TrembitaCluster::leave`] failed.
...
/// Why a cluster-wide [`scale_cluster`](TrembitaCluster::scale_cluster) failed.
#[derive(Debug, thiserror::Error)]
...
/// Why [`TrembitaCluster::add_raft_groups`] failed.
#[derive(Debug, thiserror::Error)]
