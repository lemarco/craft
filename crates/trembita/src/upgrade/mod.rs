...
//!
//! Reference [`UpgradeMachine`] in `trembita-core`, leader reconcile + local executor in
//! [`spawn_upgrade_coordinator`], HTTP hooks via `trembita-http` [`UpgradeApi`](trembita_http::UpgradeApi).
...
use trembita_core::UpgradeMachine as UpgradeStateMachineAlias;
...
pub async fn report_upgrade_boot(
    cluster: &TrembitaCluster<UpgradeStateMachineAlias>,
) -> Result<(), UpgradeRunError> {
    let members: Vec<trembita_proto::NodeId> = cluster.members().to_vec();
        ...
        && !view.completed.contains(&cluster.node_id())
    {
        propose_upgrade(
            cluster,
            trembita_core::UpgradeCommand::Report {
                node_id: cluster.node_id(),
                phase: trembita_core::UpgradePhase::Ready,
            },
        )
        .await?;
    }
...
pub fn spawn_upgrade_runtime(
    cluster: Arc<TrembitaCluster<UpgradeStateMachineAlias>>,
    opts: UpgradeOpts,
