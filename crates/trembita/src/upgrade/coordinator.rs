//! Leader reconcile + local executor loop for [`UpgradeMachine`](trembita_core::UpgradeMachine).
...
impl UpgradeOpts {
    /// Sensible defaults under `/opt/trembita/bin`.
    #[must_use]
    pub fn default_paths() -> Self {
        Self {
            install_dir: PathBuf::from("/opt/trembita/bin"),
            current_link: PathBuf::from("/opt/trembita/bin/current"),
...
async fn leader_reconcile(
    ...
    members: &[NodeId],
    view: &trembita_core::UpgradeView,
) -> Result<(), UpgradeRunError> {
