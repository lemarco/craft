//! Facade persistence: `TrembitaClusterBuilder::data_dir` survives stop → restart.
...
use trembita_test_support::{
    ...
    await_trembita_leader, fast_raft_config_with_seed, wait_for_group_leaders,
...
where
    M: trembita::core::StateMachine + Send + Sync + Default + 'static,
{
    TrembitaCluster::builder(id, machine)
        .members(members)
...
async fn compacted_snapshot_survives_facade_restart() {
    ...
    {
        let cluster = TrembitaCluster::builder(node_id, TrackedKv::default())
            .members([node_id])
...
async fn three_node_majority_survives_one_member_restart() {
    ...
    let mut clusters: Vec<Arc<TrembitaCluster<KvMachine>>> = Vec::new();
    for &id in &members {
    ...
    wait_for_trembita_stopped(follower.as_ref()).await;
    drop(follower);
