//! Multi-Raft via [`TrembitaClusterBuilder::raft_machines`] (write-sharding-multi-raft).
...
async fn wait_for_all_group_leaders(clusters: &[Arc<TrembitaCluster<KvMachine>>]) {
...
async fn cluster_leader(
    clusters: &[Arc<TrembitaCluster<KvMachine>>],
) -> Arc<TrembitaCluster<KvMachine>> {
    await_trembita_leader(clusters).await
}

async fn leave_via_cluster_rpc(joiner: &TrembitaCluster<KvMachine>, joiner_id: NodeId) {
...
async fn join_fourth_node(
    ...
    ids: [NodeId; 3],
    clusters: &[Arc<TrembitaCluster<KvMachine>>],
) -> (NodeId, TrembitaCluster<KvMachine>) {
    wait_for_all_group_leaders(clusters).await;
    let leader = cluster_leader(clusters).await;
