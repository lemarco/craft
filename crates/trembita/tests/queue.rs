...
fn shutdown_queue_cluster(net: &LocalNetwork, clusters: Vec<Arc<TrembitaCluster<KvMachine>>>) {
    for c in &clusters {
        c.shutdown();
    }
    drop(clusters);
...
async fn membership_autoscale_invokes_join_hook() {
    ...
    let joiner: Arc<Mutex<Option<Arc<TrembitaCluster<KvMachine>>>>> = Arc::new(Mutex::new(None));
