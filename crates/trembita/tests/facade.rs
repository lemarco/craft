//! End-to-end tests for the [`TrembitaCluster`] facade over the in-memory
...
/// auto-worker group `"w"`.
async fn spawn_cluster() -> (LocalNetwork, Vec<Arc<TrembitaCluster<Kv>>>) {
...
async fn wait_for_directory_workers(leader: &TrembitaCluster<Kv>, count: usize) {
...
async fn pick_follower(clusters: &[Arc<TrembitaCluster<Kv>>]) -> Arc<TrembitaCluster<Kv>> {
    for c in clusters {
...
async fn spawn_reachability_cluster() -> (LocalNetwork, Vec<Arc<TrembitaCluster<Kv>>>) {
...
async fn telemetry_publishes_consensus_and_actor_metrics() {
    ...
    eventually_default("consensus + actor metrics on leader", move || {
        ...
        out.contains("trembita_raft_is_leader")
            && out.contains("trembita_actor_spawns_total")
            && out.contains("trembita_actor_instances{actor=\"w\"}")
    })
    .await;
...
async fn crash_driven_reconcile_reacts_to_reachability_not_membership() {
    ...
    let leader_node = await_trembita_leader(&clusters).await;
