...
//!
//! Mirrors the saga facade: [`CraftCluster::run_keyed_2pc`] /
//! [`CraftCluster::resume_cross_shard_2pc`] with a durable client journal on
//! Meta-Raft (and optional Redis when configured).
//!
//! Run with: `cargo run -p craft --example cross_shard_2pc`
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plan = TwoPhasePlan {
        ...
        steps: vec![
            TwoPhaseStep {
                key: key_a,
                command: craft::proto::encode(&Cmd::Set {
                    key: "from".into(),
                    value: "100".into(),
                })?,
            },
