...
//!
//! Run with: `cargo run -p craft --example three_node_local`

use std::collections::BTreeMap;
use std::sync::Arc;
use std::time::Duration;

use craft::client::{RetryPolicy, TypedClient};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let remote =
        craft::client::RemoteClient::new(Arc::new(net.clone()), ids).with_retry(RetryPolicy {
            max_attempts: 8,
