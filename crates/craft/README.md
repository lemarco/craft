...
`CraftCluster::builder` assembles a whole node — consensus runtime, actors,
...
- Runnable examples: `cargo run -p craft --example kv_store`
  (also `three_node_local`, `actors_cluster`).
- The reference runner binary: [`craft-node`](https://crates.io/crates/craft-node).
