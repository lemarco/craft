...
impl Observer for Fake {
    ...
    fn queues(&self) -> BoxFuture<'_, QueuesView> {
        Box::pin(async move {
            QueuesView {
                streams: vec![trembita_dashboard::QueueStreamView {
                    stream: "jobs".into(),
                    pending: 5,
                    leased: 2,
                    dead_letter: 1,
...
async fn metrics_render_prometheus_text() {
    ...
    assert_eq!(status, 200);
    assert!(body.contains("# TYPE trembita_client_requests_total counter"));
    assert!(body.contains("trembita_client_requests_total 7"));
}
...
async fn dashboard_serves_html() {
    ...
    assert_eq!(status, 200);
    assert!(body.contains("<title>trembita · dashboard</title>"));
}
...
async fn sse_feed_streams_emitted_events() {
    ...
    let _ = events.emit(trembita_dashboard::TrembitaEvent::LeaderChanged { term: 5, leader: 3 });
