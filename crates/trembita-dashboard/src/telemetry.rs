...
//!
//! The runtime emits [`TrembitaEvent`]s onto an [`EventBus`] (a bounded broadcast
...
/// Why an actor stopped (surfaced in [`TrembitaEvent::ActorStopped`]).
...
pub struct TraceOpts {
    /// Emit a [`TrembitaEvent::MessageTraced`] per handled message.
    pub messages: bool,
...
/// A bounded broadcast bus for [`TrembitaEvent`]s.
///
...
#[derive(Clone)]
pub struct EventBus {
    tx: broadcast::Sender<TrembitaEvent>,
...
impl EventBus {
    ...
    #[must_use]
    pub fn emit(&self, event: TrembitaEvent) -> usize {
...
pub struct EventSubscription {
    rx: broadcast::Receiver<TrembitaEvent>,
    dropped: Arc<AtomicU64>,
}
...
impl EventSubscription {
    ...
    pub async fn recv(&mut self) -> Option<TrembitaEvent> {
        loop {
...
mod tests {
    ...
    async fn subscribers_receive_emitted_events() {
        ...
        assert_eq!(bus.emit(TrembitaEvent::NodeJoined { node_id: 3 }), 1);
        assert_eq!(
            sub.recv().await,
            Some(TrembitaEvent::NodeJoined { node_id: 3 })
        );
    }
    ...
    fn events_serialize_to_tagged_json() {
        let json =
            serde_json::to_string(&TrembitaEvent::LeaderChanged { term: 4, leader: 2 }).unwrap();
        assert_eq!(json, r#"{"event":"leader_changed","term":4,"leader":2}"#);
        let json = serde_json::to_string(&TrembitaEvent::JobEnqueued {
            stream: "jobs".into(),
            job_id: 7,
        })
        .unwrap();
