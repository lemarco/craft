...
use crate::{
    CatalogCommand, LogId, LogIndex, NodeId, QueueAutoscalePolicyCommand, Round,
...
pub enum EntryPayload {
    ...
    /// Multi-Raft catalog metadata (dynamic catalog; applied by trembita, not the user SM).
    Catalog(CatalogCommand),
    /// Cross-shard saga journal metadata (Meta-Raft saga journal; applied by trembita, not the user SM).
    SagaJournal(SagaJournalCommand),
    /// Cross-shard 2PC prepare staging (durable 2PC; applied by trembita, not the user SM).
    TwoPhasePrepare(TwoPhasePrepareCommand),
    /// Cross-shard 2PC abort tombstone (durable 2PC; applied by trembita, not the user SM).
    TwoPhaseAbort(TwoPhaseAbortCommand),
    /// Cross-shard 2PC client journal metadata (Meta-Raft; applied by trembita, not the user SM).
    TwoPhaseJournal(TwoPhaseJournalCommand),
    /// Job queue autoscale policy (Meta-Raft; applied by trembita, not the user SM).
