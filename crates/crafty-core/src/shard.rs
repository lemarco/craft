...
fn group_node_weight(group: RaftGroupId, node: crafty_proto::NodeId) -> u64 {
...
pub(crate) fn place_group(
    group: RaftGroupId,
    nodes: &[crafty_proto::NodeId],
) -> Option<crafty_proto::NodeId> {
    nodes
        .iter()
        .copied()
...
pub fn group_host_assignment(
    groups: &[RaftGroupId],
    nodes: &[crafty_proto::NodeId],
) -> BTreeMap<RaftGroupId, crafty_proto::NodeId> {
...
pub fn group_voters(
    group: RaftGroupId,
    live_nodes: &[crafty_proto::NodeId],
    replication_factor: u32,
) -> Vec<crafty_proto::NodeId> {
...
pub struct GroupReplicationTarget {
    ...
    pub voters: Vec<crafty_proto::NodeId>,
    /// Non-voting learners (catch-up replicas).
    pub learners: Vec<crafty_proto::NodeId>,
}
...
pub fn group_membership_assignment(
    ...
    live_nodes: &[crafty_proto::NodeId],
    replication_factor: u32,
) -> BTreeMap<RaftGroupId, Vec<crafty_proto::NodeId>> {
    groups
        .iter()
...
pub fn groups_joining_node_affects(
    node: crafty_proto::NodeId,
    all_groups: &[RaftGroupId],
    live_nodes_before: &[crafty_proto::NodeId],
    live_nodes_after: &[crafty_proto::NodeId],
    replication_factor: u32,
) -> Vec<RaftGroupId> {
    debug_assert!(
...
pub fn plan_group_membership_sync(
    catalog: &[RaftGroupId],
    live_nodes: &[crafty_proto::NodeId],
    current_voters: &BTreeMap<RaftGroupId, Vec<crafty_proto::NodeId>>,
    current_learners: &BTreeMap<RaftGroupId, Vec<crafty_proto::NodeId>>,
    replication_factor: u32,
    learner_factor: u32,
