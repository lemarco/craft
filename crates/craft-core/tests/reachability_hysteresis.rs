...
use craft_core::{Config, FailureDetectorKind, ReachabilityConfig, Role};
use craft_proto::{AppendEntriesReply, NodeId, RaftRpcReply, RequestVoteReply, Round, Term};
...
fn leader() -> craft_core::RaftNode {
    let mut n = craft_core::RaftNode::new(NodeId(1), [NodeId(1), NodeId(2), NodeId(3)], cfg());
    n.campaign();
...
fn ack(n: &mut craft_core::RaftNode, from: u64, round: u64) {
    n.receive_reply(
        NodeId(from),
