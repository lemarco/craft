...
//! process-wide default, so embedding crafty never fights the host app over
...
/// ALPN protocol identifier negotiated on every crafty QUIC connection.
pub const ALPN: &[u8] = b"crafty/1";

/// The Common Name prefix used to bind a certificate to a [`NodeId`]
/// (`crafty-node-<id>`).
pub const NODE_CN_PREFIX: &str = "crafty-node-";
...
impl ClusterCa {
    ...
    pub fn generate() -> Result<Self, TlsError> {
        ...
        params
            .distinguished_name
            .push(DnType::CommonName, "crafty cluster CA");
    ...
    /// Issue a fresh [`NodeIdentity`] for `node`, with `crafty-node-<id>` as both
...
/// The DNS server-name (and certificate CN) for a node: `crafty-node-<id>`.
#[must_use]
pub fn node_server_name(node: NodeId) -> String {
    format!("{NODE_CN_PREFIX}{}", node.0)
}

/// Parse a cluster node id from a certificate CN / DNS SAN (`crafty-node-<id>`).
#[must_use]
