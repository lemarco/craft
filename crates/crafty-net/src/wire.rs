...
//! "framing" here is thin: bodies are `postcard`-encoded `crafty-proto` types
...
use crafty_proto::{CodecError, decode, encode};
use serde::Serialize;
...
/// compatibility band ([`crafty_proto::protocol_version_compatible`]).
///
...
pub fn check_protocol_version(version: Option<u32>) -> Result<(), WireError> {
    ...
    if crafty_proto::protocol_version_compatible(got) {
        Ok(())
    } else {
        Err(WireError::ProtocolVersion {
            got,
            expected: crafty_proto::PROTOCOL_VERSION,
        })
    }
}
