...
use crafty_net::load_pem_material;
use crafty_net::{
...
fn parse_peers(raw: &str) -> Result<PeerDirectory, String> {
    ...
    if map.is_empty() {
        return Err("CRAFTY_PEERS is empty".into());
    }
    Ok(map)
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let node_id = node_from_env("CRAFTY_NODE_ID")?;
    let peers = parse_peers(&env("CRAFTY_PEERS")?)?;
    let paths = CertPaths::new(
        env("CRAFTY_NODE_CERT")?,
        env("CRAFTY_NODE_KEY")?,
        env("CRAFTY_CA_CERT")?,
    );
    let phase = env("CRAFTY_E2E_QUEUE_PHASE")?;
    let stream = queue_stream();

    let submit = node_from_env("CRAFTY_QUEUE_SUBMIT_NODE").unwrap_or(NodeId(1));
    let worker_peer = node_from_env("CRAFTY_QUEUE_WORKER_PEER").unwrap_or(NodeId(2));
    let worker_node = env("CRAFTY_QUEUE_WORKER_NODE")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(worker_peer.0);
    let worker_instance = env("CRAFTY_QUEUE_WORKER_INSTANCE")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(1);
    ...
    match phase.as_str() {
        ...
        "after_failover" => {
            let contact = node_from_env("CRAFTY_QUEUE_CONTACT_NODE").unwrap_or(submit);
            after_failover(transport, &stream, contact, worker_node, worker_instance).await?;
        }
        other => return Err(format!("unknown CRAFTY_E2E_QUEUE_PHASE {other:?}").into()),
    }
