...
//! Redis. In production, swap in `crafty_store_redis::RedisStore` — the worker
...
//! ```
//!
//! Run with: `cargo run -p crafty-store-redis --example idempotent_worker`

use std::sync::Arc;
use std::sync::atomic::{AtomicU32, Ordering};

use crafty_actor::{ActorStateStore, InMemoryStore};
