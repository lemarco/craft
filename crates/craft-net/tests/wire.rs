//! Contract tests for the transport-agnostic `craft-net` core: the route table,
...
fn peer_reply_body_round_trips() {
    let reply = RaftRpcReply::AppendEntries(craft_net::proto::AppendEntriesReply {
        term: Term(4),
        success: true,
        conflict_index: None,
        conflict_term: None,
...
fn protocol_version_missing_header_defaults_to_one() {
    ...
    assert!(check_protocol_version(Some(craft_net::proto::PROTOCOL_VERSION)).is_ok());
