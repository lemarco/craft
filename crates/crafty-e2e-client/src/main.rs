...
use crafty_net::{CertPaths, PeerDirectory, QuicTransport, client_config, client_endpoint};
use crafty_proto::NodeId;
use crafty_sim::{History, Model};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let node_id = NodeId(
        env("CRAFTY_NODE_ID")?
            .parse()
            .map_err(|_| "CRAFTY_NODE_ID must be u64")?,
    );
    ...
    let rounds: u64 = env::var("CRAFTY_LIN_ROUNDS")
        .ok()
