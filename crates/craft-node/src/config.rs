//! Environment-driven configuration for the `craft-node` reference binary.
...
use craft::discovery::Seed;
use craft::{NodeId, PeerDirectory, PemSecurity, Security, cert_paths_from_env};
use craft_actor::DEFAULT_DRAIN_TIMEOUT;
...
pub struct NodeConfig {
    ...
    /// Optional admin TLS PEM paths (`CRAFT_ADMIN_TLS_CERT` + `CRAFT_ADMIN_TLS_KEY`).
    ...
    /// On shutdown, remove this node from the cluster via [`craft::CraftCluster::leave`]
    ...
    pub pem_paths: Option<craft::CertPaths>,
...
/// Derive a node id: explicit `CRAFT_NODE_ID` wins, otherwise a Kubernetes
/// `POD_NAME` ordinal (`craft-0` → `NodeId(1)`), else `1`.
pub fn parse_node_id(
    craft_node_id: Option<&str>,
    pod_name: Option<&str>,
) -> Result<NodeId, Box<dyn Error>> {
    if let Some(raw) = craft_node_id {
...
pub fn load_security(
    ...
    if let Some(base) = pem.ordinal_base {
        let ca = pem
            .ca_cert
            .ok_or("CRAFT_CERT_ORDINAL_BASE requires CRAFT_CA_CERT")?;
...
pub fn load_security_from_env(
    ...
    load_security(
        ...
        PemEnv {
            ...
            ordinal_base: env("CRAFT_CERT_ORDINAL_BASE").as_deref(),
...
pub fn config_from_env() -> Result<NodeConfig, Box<dyn Error>> {
    ...
    let graceful_leave = env_bool("CRAFT_GRACEFUL_LEAVE");
    let admin_tls = match (env("CRAFT_ADMIN_TLS_CERT"), env("CRAFT_ADMIN_TLS_KEY")) {
        (Some(cert), Some(key)) => Some((PathBuf::from(cert), PathBuf::from(key))),
        (None, None) => None,
        _ => {
            return Err(
                "CRAFT_ADMIN_TLS_CERT and CRAFT_ADMIN_TLS_KEY must both be set or both unset"
                    .into(),
            );
        }
    };
...
/// Parse `CRAFT_DRAIN_TIMEOUT` (seconds, optional `s`/`m` suffix; default 60).
...
/// Read [`parse_drain_timeout`] from `CRAFT_DRAIN_TIMEOUT`.
pub fn drain_timeout_from_env() -> Duration {
    parse_drain_timeout(env("CRAFT_DRAIN_TIMEOUT").as_deref())
}

/// Parse a cert-watch period (`CRAFT_CERT_WATCH_SECS`, default 60).
...
pub fn cert_watch_period_from_env() -> Duration {
    parse_cert_watch_period(env("CRAFT_CERT_WATCH_SECS").as_deref())
}
...
mod tests {
    ...
    const CRAFT_ENV_KEYS: &[&str] = &[
        "CRAFT_NODE_ID",
        "POD_NAME",
        "CRAFT_LISTEN",
        "CRAFT_ADMIN",
        "CRAFT_ADMIN_TLS_CERT",
        "CRAFT_ADMIN_TLS_KEY",
        "CRAFT_PEERS",
        "CRAFT_JOIN_SEEDS",
        "CRAFT_DISCOVERY",
    ...
    fn explicit_pem_paths_load_for_single_node() {
        ...
        with_craft_env(
            &[
                ("CRAFT_NODE_CERT", Some(node_cert.to_str().unwrap())),
                ("CRAFT_NODE_KEY", Some(node_key.to_str().unwrap())),
                ("CRAFT_CA_CERT", Some(ca_cert.to_str().unwrap())),
            ],
            || {
    ...
    fn cert_manager_ordinal_base_resolves_pem_paths() {
        ...
        with_craft_env(
            &[
                ("POD_NAME", Some("craft-2")),
                ("CRAFT_CERT_ORDINAL_BASE", Some(base.to_str().unwrap())),
                ("CRAFT_CA_CERT", Some(base.join("ca.pem").to_str().unwrap())),
            ],
            || {
                let cfg = config_from_env().expect("config");
    ...
    #[test]
    fn node_id_prefers_craft_node_id_over_pod_name() {
        with_craft_env(
            &[("CRAFT_NODE_ID", Some("7")), ("POD_NAME", Some("craft-2"))],
    ...
    fn node_id_derives_from_pod_name_ordinal() {
        with_craft_env(&[("POD_NAME", Some("craft-0"))], || {
            assert_eq!(node_id_from_env().unwrap(), NodeId(1));
        });
        with_craft_env(&[("POD_NAME", Some("craft-2"))], || {
    ...
    fn cert_watch_period_defaults_and_parses() {
        ...
        with_craft_env(&[("CRAFT_CERT_WATCH_SECS", Some("15"))], || {
    ...
    fn parse_discovery_parses_dns_spec() {
        let spec = parse_discovery("dns:craft:craft-headless:3:7443").expect("discovery");
        assert_eq!(spec.prefix, "craft");
        assert_eq!(spec.service, "craft-headless");
    ...
    fn config_from_env_honours_admin_disable_and_join_flags() {
        with_craft_env(
            &[
                ("CRAFT_ADMIN", Some("-")),
                ("CRAFT_ALLOW_JOIN", Some("yes")),
                ("CRAFT_ALLOW_LEAVE", Some("on")),
                ("CRAFT_GRACEFUL_LEAVE", Some("true")),
            ],
            || {
                let cfg = config_from_env().expect("config");
