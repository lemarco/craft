//! Contract tests for the transport-agnostic `trembita-net` core: the route table,
...
use trembita_net::route::{self, Route, TrafficClass};
use trembita_net::wire::{
    self, CONTENT_TYPE, MAX_BODY_BYTES, check_content_type, check_protocol_version, decode_body,
    encode_body,
};
use trembita_net::{PeerDirectory, WireError};
...
fn peer_reply_body_round_trips() {
    let reply = RaftRpcReply::AppendEntries(trembita_net::proto::AppendEntriesReply {
        term: Term(4),
        success: true,
        conflict_index: None,
        conflict_term: None,
...
fn protocol_version_missing_header_defaults_to_one() {
    ...
    assert!(check_protocol_version(Some(trembita_net::proto::PROTOCOL_VERSION)).is_ok());
