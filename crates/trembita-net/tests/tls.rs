...
use trembita_net::proto::NodeId;
use trembita_net::tls::{ClusterCa, client_config, node_server_name, server_config};
...
async fn wrong_server_name_is_rejected() {
    ...
    let attempt = client
        .connect_with(client_cfg, server_addr, "trembita-node-999")
        .unwrap();
