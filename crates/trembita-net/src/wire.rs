...
//! "framing" here is thin: bodies are `postcard`-encoded `trembita-proto` types
...
use trembita_proto::{CodecError, decode, encode};
...
/// compatibility band ([`trembita_proto::protocol_version_compatible`]).
///
...
pub fn check_protocol_version(version: Option<u32>) -> Result<(), WireError> {
    ...
    if trembita_proto::protocol_version_compatible(got) {
        ...
        Err(WireError::ProtocolVersion {
            got,
            expected: trembita_proto::PROTOCOL_VERSION,
        })
    }
}
