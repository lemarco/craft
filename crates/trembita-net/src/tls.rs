...
//! process-wide default, so embedding trembita never fights the host app over
...
/// ALPN protocol identifier negotiated on every trembita QUIC connection.
pub const ALPN: &[u8] = b"trembita/1";

/// The Common Name prefix used to bind a certificate to a [`NodeId`]
/// (`trembita-node-<id>`).
pub const NODE_CN_PREFIX: &str = "trembita-node-";
...
impl ClusterCa {
    ...
    pub fn generate() -> Result<Self, TlsError> {
        ...
        params
            .distinguished_name
            .push(DnType::CommonName, "trembita cluster CA");
    ...
    /// Issue a fresh [`NodeIdentity`] for `node`, with `trembita-node-<id>` as both
...
/// The DNS server-name (and certificate CN) for a node: `trembita-node-<id>`.
#[must_use]
pub fn node_server_name(node: NodeId) -> String {
    format!("{NODE_CN_PREFIX}{}", node.0)
}

/// Parse a cluster node id from a certificate CN / DNS SAN (`trembita-node-<id>`).
#[must_use]
