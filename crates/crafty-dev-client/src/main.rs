...
use crafty_client::{Client, RemoteClient, RetryPolicy};
...
fn demo_config() -> Result<DemoConfig, String> {
    ...
    Ok(DemoConfig {
        ...
        worker_peer,
        worker_node: env("CRAFTY_DEMO_WORKER_NODE")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(worker_peer.0),
        worker_instance: env("CRAFTY_DEMO_WORKER_INSTANCE")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(1),
        stream: env::var("CRAFTY_JOB_QUEUE")
            .ok()
            .filter(|s| !s.is_empty())
...
fn watch_pause_secs() -> u64 {
    env::var("CRAFTY_DEV_WATCH_PAUSE_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(10)
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mode = env::args()
        .nth(1)
        .or_else(|| env::var("CRAFTY_DEV_MODE").ok())
    ...
    println!("crafty dev client (node {}, mode={mode})", node_id.0);
